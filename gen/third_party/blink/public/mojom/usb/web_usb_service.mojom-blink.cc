...
uint32_t WebUsbService::ForgetDevice_Sym::IPCStableHash() {
  ...
  // hash instead of __LINE__.
  constexpr uint32_t kHash = base::MD5Hash32Constexpr(
          "(Impl)blink::mojom::WebUsbService::ForgetDevice");
  const uint32_t hash = kHash;
  base::debug::Alias(&hash);
  return hash;
}
...
void WebUsbServiceProxy::ForgetDevice(
    const WTF::String& in_guid, ForgetDeviceCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1(
  ...
  mojo::Message message(
      internal::kWebUsbService_ForgetDevice_Name, kFlags, 0, 0, nullptr);
  mojo::internal::MessageFragment<
...
void WebUsbServiceAsyncWaiter::ForgetDevice(
  ...
  base::RunLoop loop;
  proxy_->ForgetDevice(std::move(guid),
      base::BindOnce(
          [](base::RunLoop* loop) {
            loop->Quit();
          },
