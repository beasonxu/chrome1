...
uint32_t LockScreenService::GetKeys_Sym::IPCStableHash() {
  ...
  constexpr uint32_t kHash = base::MD5Hash32Constexpr(
          "(Impl)blink::mojom::LockScreenService::GetKeys");
...
uint32_t LockScreenService::SetData_Sym::IPCStableHash() {
  ...
  // hash instead of __LINE__.
  constexpr uint32_t kHash = base::MD5Hash32Constexpr(
          "(Impl)blink::mojom::LockScreenService::SetData");
  const uint32_t hash = kHash;
  base::debug::Alias(&hash);
  return hash;
}
...
void LockScreenServiceProxy::GetKeys(
  ...
  mojo::Message message(
      internal::kLockScreenService_GetKeys_Name, kFlags, 0, 0, nullptr);
...
void LockScreenServiceProxy::SetData(
    const std::string& in_key, const std::string& in_data, SetDataCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1(
  ...
  mojo::Message message(
      internal::kLockScreenService_SetData_Name, kFlags, 0, 0, nullptr);
  mojo::internal::MessageFragment<
  ...
  proxy_->GetKeys(
      base::BindOnce(
          [](base::RunLoop* loop,
             std::vector<std::string>* out_keys
,
             const std::vector<std::string>& keys) {*out_keys = std::move(keys);
            loop->Quit();
          },
          &loop,
          out_keys));
...
void LockScreenServiceAsyncWaiter::SetData(
    const std::string& key, const std::string& data, LockScreenServiceStatus* out_status) {
  base::RunLoop loop;
