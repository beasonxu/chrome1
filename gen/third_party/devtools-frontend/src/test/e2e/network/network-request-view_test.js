...
(0, mocha_extensions_js_1.describe)('The Network Request view', async () => {
    ...
    (0, mocha_extensions_js_1.it)('can create header overrides via context menu', async () => {
        ...
        const createHeaderOverrideMenuEntry = await (0, helper_js_1.waitForAria)('Create response header override');
        await (0, helper_js_1.click)(createHeaderOverrideMenuEntry);
        const infoBar = await (0, helper_js_1.waitForAria)('Select a folder to store override files in.');
        const button = await (0, helper_js_1.waitFor)('.infobar-main-row .infobar-button', infoBar);
        await (0, helper_js_1.click)(button);
        await (0, helper_js_1.waitFor)('devtools-button.add-block');
        const folderElement = await (0, helper_js_1.waitFor)('.tree-outline-disclosure ol.tree-outline .navigator-fs-folder-tree-item');
        const textContent = await folderElement.evaluate(el => el.textContent || '');
        chai_1.assert.match(textContent, new RegExp(`localhost(:|%3A)${(0, helper_js_1.getTestServerPort)()}/test/e2e/resources/network`));
        await (0, helper_js_1.waitFor)('.tabbed-pane-header-tab[aria-label=".headers"]');
    });
