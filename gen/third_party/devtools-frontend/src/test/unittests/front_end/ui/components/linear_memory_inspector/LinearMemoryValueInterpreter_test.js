...
describeWithLocale('LinearMemoryValueInterpreter', () => {
    function setUpComponent() {
        ...
        component.data = {
            ...
            valueTypes: new Set(["Integer 8-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int8 */]),
    ...
    it('listens on TypeToggleEvents', async () => {
        ...
        const expectedType = "Float 64-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Float64 */;
    ...
    it('renders the endianness options', () => {
        ...
        assert.deepEqual(input.value, "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */);
        const options = input.querySelectorAll('option');
        const endiannessSettings = Array.from(options).map(option => option.value);
        assert.deepEqual(endiannessSettings, [
            "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */,
            "Big Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Big */,
        ]);
    });
    it('triggers an event on changing endianness', async () => {
        ...
        const event = await eventPromise;
        assert.deepEqual(event.data, "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */);
    });
});
