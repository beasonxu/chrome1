...
describeWithRealConnection('ObjectPropertiesSection', () => {
    async function setupTreeOutline(code, accessorPropertiesOnly, generatePreview, nonIndexedPropertiesOnly) {
        ...
        const { properties } = await result.object.getAllProperties(accessorPropertiesOnly, generatePreview, nonIndexedPropertiesOnly);
...
describeWithMockConnection('ObjectPropertiesSection', () => {
    ...
    it('appends a memory icon for DWARF inspectable objects', () => {
        ...
        const valueNode = new Bindings.DebuggerLanguagePlugins.ValueNode(callFrame, undefined, 'object', undefined, undefined, 2 /* inspectableAddress*/);
        const div = document.createElement('div');
        assert.isFalse(div.hasChildNodes());
