...
describeWithRealConnection('BreakpointManager', () => {
    ...
    class TestDebuggerModel extends SDK.DebuggerModel.DebuggerModel {
        ...
        async setBreakpointByURL(_url, _lineNumber, _columnNumber, _condition) {
            return Promise.resolve({ breakpointId: BREAKPOINT_ID, locations: [new SDK.DebuggerModel.Location(this, SCRIPT_ID, 42)] });
        }
    ...
    function createFakeScriptMapping(debuggerModel, uiSourceCode, SCRIPT_ID) {
        const sdkLocation = new SDK.DebuggerModel.Location(debuggerModel, SCRIPT_ID, 13);
    ...
    async function runBreakpointMovedTest(fileSystem) {
        ...
        const networkURL = 'http://www.google.com/example.js';
        ...
        const script = new SDK.Script.Script(debuggerModel, SCRIPT_ID, networkURL, 0, 0, 43, 0, 0, '0', true, false, undefined, false, 10, null, null, null, null, null, null);
    ...
    it('allows awaiting the restoration of breakpoints with language plugins', async () => {
        ...
        const { uiSourceCode, project } = createContentProviderUISourceCode({ url: 'test.cc', mimeType: JS_MIME_TYPE });
        ...
        const script = debuggerModel.parsedScriptSource(SCRIPT_ID, URL, 0, 0, 0, 0, 0, '', undefined, false, undefined, false, false, 0, null, null, null, null, null, null);
    ...
    it('allows awaiting on scheduled update in debugger', async () => {
        ...
        assert.isTrue(result === "OK" /* Bindings.BreakpointManager.DebuggerUpdateResult.OK */);
    ...
    describe('with persistence network overrides enabled', () => {
        ...
        it('can wait for breakpoints in overrides to be mapped to network ui source code', async () => {
            ...
            const url = 'file://path/to/overrides/www.google.com/example.js';
    ...
    describe('Breakpoints', () => {
        ...
        it('are removed and kept in storage after a back-end error', async () => {
            ...
            sinon.stub(debuggerModel, 'setBreakpointByURL').callsFake(() => {
                return Promise.resolve({ locations: [], breakpointId: null });
            });
            const mapping = createFakeScriptMapping(debuggerModel, uiSourceCode, SCRIPT_ID);
            Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().addSourceMapping(mapping);
            const breakpoint = await breakpointManager.setBreakpoint(uiSourceCode, 42, 0, '', true);
