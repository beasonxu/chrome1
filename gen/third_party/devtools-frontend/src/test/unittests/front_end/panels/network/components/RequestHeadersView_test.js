...
import { assertElement, assertShadowRoot, dispatchClickEvent, dispatchCopyEvent, dispatchKeyDownEvent, getCleanTextContentFromElements, getElementWithinComponent, renderElementIntoDOM, } from '../../../helpers/DOMHelpers.js';
...
describeWithMockConnection('RequestHeadersView', () => {
    ...
    it('renders detailed reason for blocked requests', async () => {
        ...
        const responseHeadersCategory = component.shadowRoot.querySelector('[aria-label="Response Headers"]');
        assertElement(responseHeadersCategory, HTMLElement);
        assert.strictEqual(getCleanTextContentFromElements(responseHeadersCategory, '.header-name')[4], 'not-setcross-origin-resource-policy:');
        assert.strictEqual(getCleanTextContentFromElements(responseHeadersCategory, '.header-value')[4], '');
        assert.strictEqual(getCleanTextContentFromElements(responseHeadersCategory, '.call-to-action')[0], 'To use this resource from a different origin, the server needs to specify a cross-origin ' +
    ...
    it('renders provisional headers warning', async () => {
        ...
        const requestHeadersCategory = component.shadowRoot.querySelector('[aria-label="Request Headers"]');
        assertElement(requestHeadersCategory, HTMLElement);
        assert.strictEqual(getCleanTextContentFromElements(requestHeadersCategory, '.call-to-action')[0], 'Provisional headers are shown. Disable cache to see full headers. Learn more');
    });
    ...
    it('displays decoded "x-client-data"-header', async () => {
        ...
        const requestHeadersCategory = component.shadowRoot.querySelector('[aria-label="Request Headers"]');
        assertElement(requestHeadersCategory, HTMLElement);
        assert.strictEqual(getCleanTextContentFromElements(requestHeadersCategory, '.header-name')[0], 'x-client-data:');
        assert.isTrue((getCleanTextContentFromElements(requestHeadersCategory, '.header-value')[0])
            .startsWith('CJa2yQEIpLbJAQiTocsB'));
        assert.strictEqual(getCleanTextContentFromElements(requestHeadersCategory, '.header-value code')[0], 'message ClientVariations {// Active client experiment variation IDs.repeated int32 variation_id = [3300118, 3300132, 3330195];\n}');
    });
    it('displays info about blocked "Set-Cookie"-headers', async () => {
        ...
        const responseHeadersCategory = component.shadowRoot.querySelector('[aria-label="Response Headers"]');
        assertElement(responseHeadersCategory, HTMLElement);
        assert.strictEqual(getCleanTextContentFromElements(responseHeadersCategory, '.header-name')[0], 'Set-Cookie:');
        assert.strictEqual(getCleanTextContentFromElements(responseHeadersCategory, '.header-value')[0], 'secure=only; Secure');
