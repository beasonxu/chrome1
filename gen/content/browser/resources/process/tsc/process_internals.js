...
function frameToTreeItem(frame) {
    ...
    let itemLabel = `Frame[${frame.processId}:${frame.routingId}:${frame.agentSchedulingGroupId}]:`;
...
function loadIsolatedOriginInfo() {
    ...
    pageHandler.getUserTriggeredIsolatedOrigins().then((response) => {
        ...
        const userOrigins = document.querySelector('#user-triggered-isolated-origins');
        assert(userOrigins);
    ...
    pageHandler.getWebTriggeredIsolatedOrigins().then((response) => {
        ...
        const webOrigins = document.querySelector('#web-triggered-isolated-origins');
        assert(webOrigins);
