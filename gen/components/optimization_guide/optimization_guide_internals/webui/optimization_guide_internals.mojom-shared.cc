...
bool Page_OnLogMessageAdded_Params_Data::Validate(
  ...
  // NOTE: The memory backing |object| may be smaller than |sizeof(*object)| if
  // the message comes from an older version.
  [[maybe_unused]] const Page_OnLogMessageAdded_Params_Data* object =
      static_cast<const Page_OnLogMessageAdded_Params_Data*>(data);
  ...
  if (!::optimization_guide_common::mojom::internal::LogSource_Data
        ::Validate(object->log_source, validation_context))
    return false;
  ...
  if (!mojo::internal::ValidatePointerNonNullable(
          object->message, 5, validation_context)) {
    return false;
  }
