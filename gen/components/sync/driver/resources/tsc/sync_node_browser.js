...
function updateNodeDetailView(node) {
    const nodeDetailsView = document.querySelector('#node-details');
...
function setLastRefreshTime(str) {
    const refreshTime = document.querySelector('#node-browser-refresh-time');
    assert(refreshTime);
 ...
 */
function clear() {
    const treeContainer = document.querySelector('#sync-node-tree-container');
 ...
 */
function refresh() {
    const refreshButton = document.querySelector('#node-browser-refresh-button');
