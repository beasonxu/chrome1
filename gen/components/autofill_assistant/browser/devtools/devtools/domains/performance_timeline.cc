...
#include "components/autofill_assistant/browser/devtools/devtools/domains/performance_timeline.h"
...
void Domain::RegisterEventHandlersIfNeeded() {
  ...
  dispatcher_->RegisterEventHandler(
      ...
      base::BindRepeating(&Domain::DispatchTimelineEventAddedEvent,
...
void ExperimentalDomain::Enable(
  ...
  dispatcher_->SendMessage("PerformanceTimeline.enable", params->Serialize(), optional_node_frame_id, base::BindOnce(&Domain::HandleEnableResponse, std::move(callback)));
}
...
void Domain::DispatchTimelineEventAddedEvent(const base::Value& params) {
  ErrorReporter errors;
  std::unique_ptr<TimelineEventAddedParams> parsed_params(TimelineEventAddedParams::Parse(params, &errors));
  DCHECK(!errors.HasErrors()) << errors.ToString();
  for (ExperimentalObserver& observer : observers_) {
    observer.OnTimelineEventAdded(*parsed_params);
  }
}
