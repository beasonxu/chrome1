...
#include "components/autofill_assistant/browser/devtools/devtools/internal/type_conversions_dom_storage.h"
...
std::unique_ptr<ClearParams> ClearParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (storage_id_value) {
    errors->SetName("storageId");
    result->storage_id_ = internal::FromValue<::autofill_assistant::dom_storage::StorageId>::Parse(*storage_id_value, errors);
  } else {
