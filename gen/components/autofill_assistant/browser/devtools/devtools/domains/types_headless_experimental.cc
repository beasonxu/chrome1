...
#include "components/autofill_assistant/browser/devtools/devtools/internal/type_conversions_headless_experimental.h"
...
std::unique_ptr<ScreenshotParams> ScreenshotParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (format_value) {
    ...
    result->format_ = internal::FromValue<::autofill_assistant::headless_experimental::ScreenshotParamsFormat>::Parse(*format_value, errors);
  }
...
std::unique_ptr<BeginFrameParams> BeginFrameParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (screenshot_value) {
    ...
    result->screenshot_ = internal::FromValue<::autofill_assistant::headless_experimental::ScreenshotParams>::Parse(*screenshot_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
