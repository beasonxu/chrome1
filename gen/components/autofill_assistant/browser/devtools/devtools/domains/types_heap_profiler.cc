...
#include "components/autofill_assistant/browser/devtools/devtools/internal/type_conversions_heap_profiler.h"
...
std::unique_ptr<SamplingHeapProfileNode> SamplingHeapProfileNode::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (children_value) {
    ...
    result->children_ = internal::FromValue<std::vector<std::unique_ptr<::autofill_assistant::heap_profiler::SamplingHeapProfileNode>>>::Parse(*children_value, errors);
  } else {
...
std::unique_ptr<SamplingHeapProfile> SamplingHeapProfile::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (head_value) {
    errors->SetName("head");
    result->head_ = internal::FromValue<::autofill_assistant::heap_profiler::SamplingHeapProfileNode>::Parse(*head_value, errors);
  } else {
  ...
  if (samples_value) {
    ...
    result->samples_ = internal::FromValue<std::vector<std::unique_ptr<::autofill_assistant::heap_profiler::SamplingHeapProfileSample>>>::Parse(*samples_value, errors);
  } else {
...
std::unique_ptr<GetSamplingProfileResult> GetSamplingProfileResult::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (profile_value) {
    ...
    result->profile_ = internal::FromValue<::autofill_assistant::heap_profiler::SamplingHeapProfile>::Parse(*profile_value, errors);
  } else {
...
std::unique_ptr<StopTrackingHeapObjectsParams> StopTrackingHeapObjectsParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  const base::Value* capture_numeric_value_value = value.FindKey("captureNumericValue");
  if (capture_numeric_value_value) {
    errors->SetName("captureNumericValue");
    result->capture_numeric_value_ = internal::FromValue<bool>::Parse(*capture_numeric_value_value, errors);
  }
  const base::Value* expose_internals_value = value.FindKey("exposeInternals");
  if (expose_internals_value) {
    errors->SetName("exposeInternals");
    result->expose_internals_ = internal::FromValue<bool>::Parse(*expose_internals_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
...
std::unique_ptr<base::Value> StopTrackingHeapObjectsParams::Serialize() const {
  ...
  if (capture_numeric_value_)
    result->Set("captureNumericValue", internal::ToValue(capture_numeric_value_.value()));
  if (expose_internals_)
    result->Set("exposeInternals", internal::ToValue(expose_internals_.value()));
  return std::move(result);
}
