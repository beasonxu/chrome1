...
#include "components/autofill_assistant/browser/devtools/devtools/internal/type_conversions_database.h"
...
std::unique_ptr<ExecuteSQLResult> ExecuteSQLResult::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (sql_error_value) {
    ...
    result->sql_error_ = internal::FromValue<::autofill_assistant::database::Error>::Parse(*sql_error_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
...
std::unique_ptr<AddDatabaseParams> AddDatabaseParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (database_value) {
    errors->SetName("database");
    result->database_ = internal::FromValue<::autofill_assistant::database::Database>::Parse(*database_value, errors);
  } else {
