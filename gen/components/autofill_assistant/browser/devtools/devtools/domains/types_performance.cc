...
#include "components/autofill_assistant/browser/devtools/devtools/internal/type_conversions_performance.h"
...
std::unique_ptr<EnableParams> EnableParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (time_domain_value) {
    ...
    result->time_domain_ = internal::FromValue<::autofill_assistant::performance::EnableTimeDomain>::Parse(*time_domain_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
...
std::unique_ptr<SetTimeDomainParams> SetTimeDomainParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (time_domain_value) {
    errors->SetName("timeDomain");
    result->time_domain_ = internal::FromValue<::autofill_assistant::performance::SetTimeDomainTimeDomain>::Parse(*time_domain_value, errors);
  } else {
...
std::unique_ptr<GetMetricsResult> GetMetricsResult::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (metrics_value) {
    ...
    result->metrics_ = internal::FromValue<std::vector<std::unique_ptr<::autofill_assistant::performance::Metric>>>::Parse(*metrics_value, errors);
  } else {
