 ...
 * passed.
 */
function setDisplayStyle(elements, displayValue) {
 ...
 *     entries div.
 */
function createUrlCard(sourcesForUrl, sourcesDiv, displayState) {
    ...
    for (const source of sourcesForUrl) {
        ...
        const sourceContainer = /** @type {!Element} */ (createElementWithClassName('div', 'source_container'));
        sourceDiv.appendChild(sourceContainer);
        createUrlHeader(source.url, source.id, sourceContainer);
        createSourceCard(source, sourceContainer, displayState.get(as64Bit(source.id)));
    }
}
/**
...
function createSourceCard(source, sourceDiv, displayState) {
    const metricElement = createElementWithClassName('div', 'entries');
    sourceDiv.appendChild(metricElement);
    const sortedEntry = source.entries.sort((x, y) => x.name.localeCompare(y.name));
...
function createEntryTable(entry, sourceDiv) {
    ...
    const sortedMetrics = entry.metrics.sort((x, y) => x.name.localeCompare(y.name));
    // Add metrics columns.
...
function urlToSourcesMapping(sources) {
    ...
    return new Map(Array.from(unsorted).sort((s1, s2) => s1[0].localeCompare(s2[0])));
}
/**
...
function addExpandToggleButton() {
    ...
    toggleExpand.addEventListener('click', () => {
        if (toggleExpand.textContent === 'Expand') {
            ...
            setDisplayStyle(document.body.querySelectorAll('.entries'), 'block');
        }
        else {
            toggleExpand.textContent = 'Expand';
            setDisplayStyle(document.body.querySelectorAll('.entries'), 'none');
        }
    });
}
/**
...
function populateThreadIds(sources) {
    const threadIdSelect = document.body.querySelector('#thread_ids');
    assert(threadIdSelect);
    const currentOptions = new Set(Array.from(threadIdSelect.options).map(o => o.value));
...
function updateUkmCache(data) {
    for (const source of data.sources) {
        ...
        if (!cachedSources.has(key)) {
            ...
            const existingEntries = new Set(cachedSources.get(key).entries.map(cachedEntry => normalizeToString(cachedEntry)));
...
function updateUkmData() {
    sendWithPromise('requestUkmData').then((/** @type {UkmData} */ data) => {
        updateUkmCache(data);
        if (document.body.querySelector('#include_cache').checked) {
        ...
        for (const el of document.getElementsByClassName('source_container')) {
            currentDisplayState.set(el.querySelector('.sourceid').textContent, el.querySelector('.entries').style.display);
        }
        const urlToSources = urlToSourcesMapping(filterSourcesUsingFormOptions(data.sources));
...
function filterSourcesUsingFormOptions(sources) {
    ...
    const newSourcesWithEntriesCleared = newSources.map(source => {
        const metricsFilterValue = document.body.querySelector('#metrics_select').value;
    ...
    const filteredSources = newSourcesWithEntriesCleared.filter(source => {
        const noUrlCheckbox = document.body.querySelector('#hide_no_url');
        assert(noUrlCheckbox);
        const noMetricsCheckbox = document.body.querySelector('#hide_no_metrics');
        assert(noMetricsCheckbox);
        return (!noUrlCheckbox.checked || source.url) &&
            (!noMetricsCheckbox.checked || source.entries.length);
    });
    const threadIds = document.body.querySelector('#thread_ids');
    assert(threadIds);
    ...
    const urlSelect = document.body.querySelector('#url_select');
    assert(urlSelect);
