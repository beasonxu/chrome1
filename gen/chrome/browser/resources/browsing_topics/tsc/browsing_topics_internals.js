...
function formatTimeDuration(totalMicrosecondsBigInt) {
...
function formatMojoTime(mojoTime) {
...
function getRealOrRandomStatusText(isRealTopic) {
    return isRealTopic ? 'Real' : 'Random';
}
function createContextDomainEntry(domain) {
    const entry = document
        .querySelector('#context-domain-entry-template').content.cloneNode(true);
    entry.querySelectorAll('span')[0].textContent = domain;
    return entry;
}
function createTopicRow(topic) {
    const row = document.querySelector('#topic-row-template').content.cloneNode(true);
    ...
    topic.observedByDomains.forEach((domain) => {
        row.querySelectorAll('td')[3].appendChild(createContextDomainEntry(domain));
    });
    return row;
}
function fieldNameFromId(id) {
...
async function asyncGetBrowsingTopicsConfiguration() {
    ...
    ['browsing-topics-enabled-div',
        ...
        assert(div);
        div.textContent += getEnabledStatusText(config[fieldNameFromId(id)]);
    });
    // Number fields
    ['number-of-epochs-to-expose-div', 'number-of-top-topics-per-epoch-div',
        ...
        assert(div);
        div.textContent +=
            config[fieldNameFromId(id)];
    });
    // Time duration fields
    ['time-period-per-epoch-div'].forEach(id => {
        const div = document.querySelector(`#${id}`);
        assert(div);
        div.textContent += formatTimeDuration(config[fieldNameFromId(id)]
            .microseconds);
    });
}
async function asyncGetBrowsingTopicsState(calculateNow) {
    ...
    if (result.overrideStatusMessage) {
        document.querySelector('#topics-state-override-status-message-div').textContent =
    ...
    result.browsingTopicsState.epochs.forEach((epoch) => {
        const epochDiv = document.querySelector('#epoch-div-template').content.cloneNode(true);
...
function createClassificationResultTopicEntry(topic) {
    const entry = document
        .querySelector('#classification-result-topic-entry-template').content
        .cloneNode(true);
    entry.querySelectorAll('span')[0].textContent = topic;
    return entry;
}
function createClassificationResultRow(host, topics) {
    const row = document
        .querySelector('#classification-result-host-row-template').content
        .cloneNode(true);
    const nestedCells = row.querySelectorAll('td');
    nestedCells[0].textContent = host;
    topics.forEach((topic) => {
        const topicText = String(topic.topicId) + '. ' + decodeString16(topic.topicName);
        nestedCells[1].appendChild(createClassificationResultTopicEntry(topicText));
    });
    return row;
}
async function asyncClassifyHosts(hosts, sequenceNumber) {
    let topicsForHosts = [];
    if (hosts.length > 0) {
        assert(pageHandler);
    ...
    for (let i = 0; i < hosts.length; i++) {
        const host = hosts[i];
        const topics = topicsForHosts[i];
        document.querySelector('#hosts-classification-result-table').appendChild(createClassificationResultRow(host, topics));
    }
...
function clearHostsClassificationResult() {
    const table = document.querySelector('#hosts-classification-result-table');
    while (table.rows[1]) {
        table.deleteRow(1);
    }
    const div = document.querySelector('#hosts-classification-input-validation-error');
    div.innerHTML = '';
    setElementVisible('hosts-classification-loader-div', false);
...
async function asyncGetModelInfo() {
    ...
    const result = response.result;
    if (result.overrideStatusMessage) {
        document.querySelector('#model-info-override-status-message-div').textContent =
            result.overrideStatusMessage.toString();
    ...
    document.querySelector('#hosts-classification-button').addEventListener('click', () => {
        clearHostsClassificationResult();
        const input = document.querySelector('#input-hosts-textarea')
            .value;
        const hosts = input.split('\n');
        const preprocessedHosts = [];
        hosts.forEach((host) => {
        ...
        preprocessedHosts.forEach((host) => {
            if (host.includes('/')) {
                inputValidationErrors.push('Host \"' + host + '" contains invalid character: "\/"');
            }
        });
        ++hostsClassificationSequenceNumber;
        if (inputValidationErrors.length > 0) {
            const errorsDiv = document.querySelector('#hosts-classification-input-validation-error');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    asyncGetModelInfo();
    document.querySelector('#refresh-topics-state-button').addEventListener('click', () => {
        asyncGetBrowsingTopicsState(/*calculateNow=*/ false);
    });
    document.querySelector('#calculate-now-button').addEventListener('click', () => {
