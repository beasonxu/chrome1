...
function addClientInfo(parent, info) {
...
function addSegmentInfoToParent(parent, segmentationKey, info) {
    const div = document.createElement('div');
    ...
    overwriteBtn.addEventListener('click', () => {
        getProxy().overwriteResult(info.segmentId, parseFloat(overwriteValue.value));
    });
...
function initialize() {
    getProxy().getCallbackRouter().onServiceStatusChanged.addListener((initialized, status) => {
        $('initialized').textContent = String(initialized);
        $('service-status').textContent = String(status);
    });
    getProxy().getCallbackRouter().onClientInfoAvailable.addListener((clientInfos) => {
