...
export class OmniboxOutput extends OmniboxElement {
    ...
    setResponsesHistory(responsesHistory) {
        ...
        this.dispatchEvent(new CustomEvent('responses-count-changed', { detail: responsesHistory.length }));
    ...
    updateSelectedResponseIndex(selection) {
        if (selection >= 0 && selection < this.responsesHistory.length) {
            ...
            this.responsesHistory[selection].forEach(this.createResultsGroup_.bind(this));
        }
    }
    prepareNewQuery() {
        this.responsesHistory.push([]);
        this.dispatchEvent(new CustomEvent('responses-count-changed', { detail: this.responsesHistory.length }));
    }
    ...
    updateAnswerImage(url, data) {
        this.autocompleteMatches.forEach(match => match.updateAnswerImage(url, data));
    }
    /** @private */
    updateDisplay_() {
    ...
    updateVisibility_() {
        ...
        this.resultsGroups_.forEach(resultsGroup => {
            resultsGroup.updateVisibility(this.displayInputs_.showIncompleteResults, this.displayInputs_.showDetails, this.displayInputs_.showAllProviders);
        });
    }
    /** @private */
    updateEliding_() {
        this.resultsGroups_.forEach(resultsGroup => resultsGroup.updateEliding(this.displayInputs_.elideCells));
    }
    /** @private */
    updateRowHeights_() {
        this.resultsGroups_.forEach(resultsGroup => resultsGroup.updateRowHeights(this.displayInputs_.thinRows));
    }
    /** @private */
    ...
    get autocompleteMatches() {
        return this.resultsGroups_.flatMap(resultsGroup => resultsGroup.autocompleteMatches);
    }
}
/**
...
class OutputResultsGroup extends OmniboxElement {
    ...
    updateVisibility(showIncompleteResults, showDetails, showAllProviders) {
        ...
        this.individualResultsList.forEach(individualResults => individualResults.hidden = !showAllProviders);
        this.innerHeaders_.forEach(innerHeader => innerHeader.hidden = !showAllProviders);
        ...
        this.autocompleteMatches.forEach(match => match.updateVisibility(showDetails));
    }
    ...
    updateRowHeights(thinRows) {
        this.autocompleteMatches.forEach(match => match.classList.toggle('thin', thinRows));
    }
    /**
     * @private
     * @return {boolean}
    ...
    get hasAdditionalProperties() {
        return this.combinedResults.hasAdditionalProperties ||
            this.individualResultsList.some(results => results.hasAdditionalProperties);
    }
    /** @return {!Array<!OutputMatch>} */
    get autocompleteMatches() {
...
class OutputResultsTable extends HTMLTableSectionElement {
    ...
    /** @return {boolean} */
    get hasAdditionalProperties() {
        return this.autocompleteMatches.some(match => match.hasAdditionalProperties);
    }
}
...
class OutputMatch extends HTMLTableRowElement {
    constructor() {
        ...
        this.addEventListener('click', () => !mouseMoved && this.classList.toggle('expanded'));
    }
    /**
    ...
    set match(match) {
        ...
        COLUMNS.forEach(column => {
            const values = column.sourceProperties.map(propertyName => /** @type {Object} */ (match)[propertyName]);
        ...
        /** @type {!OutputProperty} */
        this.additionalProperties = OutputProperty.create(ADDITIONAL_PROPERTIES_COLUMN, [unconsumedProperties]);
        this.render_();
    }
    /** @private */
    render_() {
    ...
    filter(filterText) {
        ...
        this.allProperties_.forEach(property => property.classList.remove('filtered-highlighted-nested'));
        if (!filterText) {
            return;
        }
        const matchedProperties = this.allProperties_.filter(property => FilterUtil.filterText(property.text, filterText));
        const isMatch = matchedProperties.length > 0;
        this.classList.toggle('filtered-highlighted', isMatch);
        matchedProperties.forEach(property => property.classList.add('filtered-highlighted-nested'));
    }
    /**
...
class OutputOverlappingPairProperty extends OutputPairProperty {
    ...
    render_() {
        ...
        this.notOverlapWarning_.textContent = overlap ?
            '' :
            `btw, these texts do not overlap; '${this.values_[1]}' was expected to be a suffix of '${this.values_[0]}'`;
    }
}
class OutputAnswerProperty extends FlexWrappingOutputProperty {
    ...
    render_() {
        ...
        const [image, contents, description, answer, contentsClassification, descriptionClassification] = this.values_;
        OutputAnswerProperty.renderClassifiedText_(this.contents_, /** @type {string} */ (contents), 
        /** @type {!Array<!ACMatchClassification>} */
        (contentsClassification));
        OutputAnswerProperty.renderClassifiedText_(this.description_, /** @type {string} */ (description), 
        /** @type {!Array<!ACMatchClassification>} */
    ...
    static renderClassifiedText_(container, string, classes) {
        ...
        OutputAnswerProperty.classify(string, classes)
            .map(({ string, style }) => OutputJsonProperty.renderJsonWord(string, OutputAnswerProperty.styleToClasses_(style)))
    ...
    static styleToClasses_(style) {
        ...
        return ['style-url', 'style-match', 'style-dim'].filter((_, i) => (style >> i) % 2);
    }
}
...
class OutputJsonProperty extends OutputProperty {
    ...
    render_() {
        ...
        this.text.split(/("(?:[^"\\]|\\.)*":?|\w+)/)
            .map(word => {
            return OutputJsonProperty.renderJsonWord(word, [OutputJsonProperty.classifyJsonWord(word)]);
        })
    ...
    static classifyJsonWord(word) {
        ...
        // Number.isNaN('3') === true.
        if (isNaN(word)) {
            const classification = OutputJsonProperty.classifications.find(({ re }) => re.test(word));
...
class OutputAdditionalInfoProperty extends OutputProperty {
    ...
    render_() {
        ...
        this.value.forEach(({ key, value }) => {
            this.pre_.appendChild(OutputJsonProperty.renderJsonWord(key + ': ', ['key']));
            this.pre_.appendChild(OutputJsonProperty.renderJsonWord(value + '\n', ['number']));
        });
        this.link_.href = this.createDownloadLink_();
    }
    /** @override @return {string} */
    get text() {
        return this.value.reduce((prev, { key, value }) => `${prev}${key}: ${value}\n`, '');
    }
...
class FilterUtil {
    ...
    static filterText(searchText, filterText) {
        const regexFilter = Array.from(filterText)
    ...
    static textToWords_(text) {
        ...
        if (text.length > MAX_TEXT_LENGTH) {
            ...
            console.warn(`text to be filtered too long, truncatd; max length: ${MAX_TEXT_LENGTH}, truncated text: ${text}`);
        }
...
class Column {
     ...
     */
    constructor(headerText, url, matchKey, displayAlways, tooltip, sourceProperties, outputClass) {
        ...
        const hyphenatedName = matchKey.replace(/[A-Z]/g, c => '-' + c.toLowerCase());
        /** @type {string} */
 ...
 */
const COLUMNS = [
    new Column(['Provider', 'Type'], '', 'providerAndType', true, 'Provider & Type\nThe AutocompleteProvider suggesting this result. / ' +
        'The type of the result.', ['providerName', 'type'], OutputPairProperty),
    new Column(['Relevance'], '', 'relevance', true, 'Relevance\nThe result score. Higher is more relevant.', ['relevance'], OutputTextProperty),
    new Column(['Contents', 'Description', 'Answer'], '', 'contentsAndDescription', true, 'Contents & Description & Answer\nURL classifications are styled ' +
    ...
    new Column(['sw'], '', 'swapContentsAndDescription', false, 'Swap Contents and Description', ['swapContentsAndDescription'], OutputBooleanProperty),
    new Column(['df'], '', 'allowedToBeDefaultMatch', true, 'Can be Default\nA green checkmark indicates that the result can be ' +
        'the default match (i.e., can be the match that pressing enter ' +
        'in the omnibox navigates to).', ['allowedToBeDefaultMatch'], OutputBooleanProperty),
    new Column(['bk'], '', 'starred', false, 'Bookmarked\nA green checkmark indicates that the result has been ' +
        'bookmarked.', ['starred'], OutputBooleanProperty),
    new Column(['tb'], '', 'hasTabMatch', false, 'Has Tab Match\nA green checkmark indicates that the result URL ' +
        'matches an open tab.', ['hasTabMatch'], OutputBooleanProperty),
    new Column(['URL', 'Stripped URL'], '', 'destinationUrl', true, 'URL & Stripped URL\nThe URL for the result. / The stripped URL for ' +
        'the result.', ['destinationUrl', 'isSearchType', 'strippedDestinationUrl'], OutputUrlProperty),
    new Column(['AQS Type & Subtypes'], '', 'aqsTypeSubtypes', false, 'The type and subtypes reported in the Assisted Query Stats (AQS) url ' +
        'query param.', ['aqsTypeSubtypes'], OutputTextProperty),
    new Column(['Fill', 'Inline'], '', 'fillAndInline', false, 'Fill & Inline\nThe text shown in the omnibox when the result is ' +
        'selected. / The text shown in the omnibox as a blue highlight ' +
        'selection following the cursor, if this match is shown inline.', ['fillIntoEdit', 'inlineAutocompletion'], OutputOverlappingPairProperty),
    new Column(['dl'], '', 'deletable', false, 'Deletable\nA green checkmark indicates that the result can be ' +
        'deleted from the visit history.', ['deletable'], OutputBooleanProperty),
    new Column(['pr'], '', 'fromPrevious', false, 'From Previous\nTrue if this match is from a previous result.', ['fromPrevious'], OutputBooleanProperty),
    new Column(['Tran'], 'https://cs.chromium.org/chromium/src/ui/base/page_transition_types.h' +
        '?q=page_transition_types.h&sq=package:chromium&dr=CSs&l=14', 'transition', false, 'Transition\nHow the user got to the result.', ['transition'], OutputTextProperty),
    new Column(['dn'], '', 'providerDone', false, 'Done\nA green checkmark indicates that the provider is done looking ' +
        'for more results.', ['providerDone'], OutputBooleanProperty),
    new Column(['Associated Keyword'], '', 'associatedKeyword', false, 'Associated Keyword\nIf non-empty, a "press tab to search" hint will ' +
        'be shown and will engage this keyword.', ['associatedKeyword'], OutputTextProperty),
    new Column(['Keyword'], '', 'keyword', false, 'Keyword\nThe keyword of the search engine to be used.', ['keyword'], OutputTextProperty),
    new Column(['dp'], '', 'duplicates', false, 'Duplicates\nThe number of matches that have been marked as ' +
        'duplicates of this match.', ['duplicates'], OutputTextProperty),
    new Column(['pi'], 'https://source.chromium.org/chromium/chromium/src/+/main:components/omnibox/browser/omnibox_pedal_concepts.h;l=19;drc=c741e070dbfcc33b2369e7a5131be87c7b21bb99', 'pedalId', false, 'Pedal ID\nThe ID of attached Pedal, or zero if none.', ['pedalId'], OutputTextProperty),
    new Column(['Additional Info'], '', 'additionalInfo', true, 'Additional Info\nProvider-specific information about the result.', ['additionalInfo'], OutputAdditionalInfoProperty),
];
/** @type {!Column} */
const ADDITIONAL_PROPERTIES_COLUMN = new Column(['Additional Properties'], '', 'additionalProperties', false, 'Properties not accounted for.', [], OutputJsonProperty);
const CONSUMED_SOURCE_PROPERTIES = COLUMNS.flatMap(column => column.sourceProperties);
...
customElements.define('output-pair-property', OutputPairProperty, { extends: 'td' });
customElements.define('output-overlapping-pair-property', OutputOverlappingPairProperty, { extends: 'td' });
customElements.define('output-answer-property', OutputAnswerProperty, { extends: 'td' });
customElements.define('output-boolean-property', OutputBooleanProperty, { extends: 'td' });
customElements.define('output-json-property', OutputJsonProperty, { extends: 'td' });
customElements.define('output-additional-info-property', OutputAdditionalInfoProperty, { extends: 'td' });
customElements.define('output-url-property', OutputUrlProperty, { extends: 'td' });
customElements.define('output-text-property', OutputTextProperty, { extends: 'td' });
