...
void AudioDecoderAsyncWaiter::Initialize(
  ...
  proxy_->Initialize(std::move(config),std::move(cdm_id),
      base::BindOnce(
          [](base::RunLoop* loop,
             ::media::mojom::blink::DecoderStatusPtr* out_success
,
             bool* out_needs_bitstream_conversion
,
             ::media::mojom::blink::AudioDecoderType* out_decoder_type
,
             ::media::mojom::blink::DecoderStatusPtr success,
             bool needs_bitstream_conversion,
             ::media::mojom::blink::AudioDecoderType decoder_type) {*out_success = std::move(success);*out_needs_bitstream_conversion = std::move(needs_bitstream_conversion);*out_decoder_type = std::move(decoder_type);
            loop->Quit();
          },
          &loop,
          out_success,
