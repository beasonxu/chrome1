...
import org.chromium.chrome.browser.browserservices.InstalledWebappDataRecorder_Factory;
...
import org.chromium.chrome.browser.customtabs.dependency_injection.BaseCustomTabActivityModule_ProvideInstalledWebappDataRegisterFactory;
...
public final class DaggerChromeAppComponent implements ChromeAppComponent {
  ...
  @Override
  public ClearDataDialogResultRecorder resolveClearDataDialogResultRecorder() {
  ...
  @Override
  public PermissionUpdater resolvePermissionUpdater() {
  ...
  private final class BaseCustomTabActivityComponentImpl implements BaseCustomTabActivityComponent {
    ...
    private volatile Object installedWebappDataRecorder = new MemoizedSentinel();

    private volatile Provider<InstalledWebappDataRecorder> installedWebappDataRecorderProvider;
    ...
    private InstalledWebappDataRecorder installedWebappDataRecorder() {
      Object local = installedWebappDataRecorder;
      if (local instanceof MemoizedSentinel) {
        synchronized (local) {
          local = installedWebappDataRecorder;
          if (local instanceof MemoizedSentinel) {
            local = InstalledWebappDataRecorder_Factory.newInstance(ChromeAppModule_ProvideContextFactory.provideContext(DaggerChromeAppComponent.this.chromeAppModule), BaseCustomTabActivityModule_ProvideInstalledWebappDataRegisterFactory.provideInstalledWebappDataRegister(baseCustomTabActivityModule));
            installedWebappDataRecorder = DoubleCheck.reentrantCheck(installedWebappDataRecorder, local);
          }
        }
      }
      return (InstalledWebappDataRecorder) local;
    }

    private Provider<InstalledWebappDataRecorder> installedWebappDataRecorderProvider() {
      Object local = installedWebappDataRecorderProvider;
      if (local == null) {
        local = new SwitchingProvider<>(20);
        installedWebappDataRecorderProvider = (Provider<InstalledWebappDataRecorder>) local;
      }
      return (Provider<InstalledWebappDataRecorder>) local;
    }

    private InstalledWebappRegistrar installedWebappRegistrar() {
      return new InstalledWebappRegistrar(ChromeAppModule_ProvideContextFactory.provideContext(DaggerChromeAppComponent.this.chromeAppModule), DaggerChromeAppComponent.this.resolvePermissionUpdater(), DoubleCheck.lazy(installedWebappDataRecorderProvider()));
    }
    ...
    private final class SwitchingProvider<T> implements Provider<T> {
      ...
      public T get() {
        switch (id) {
          ...
          case 20: // org.chromium.chrome.browser.browserservices.InstalledWebappDataRecorder 
          return (T) BaseCustomTabActivityComponentImpl.this.installedWebappDataRecorder();
