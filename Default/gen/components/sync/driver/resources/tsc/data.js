...
const dumpToTextButton = document.querySelector('#dump-to-text');
assert(dumpToTextButton);
const dataDump = document.querySelector('#data-dump');
assert(dataDump);
...
function serializeNode(node) {
    const includeSpecifics = document.querySelector('#include-specifics');
...
function isSelectedDatatype(type) {
    const typeCheckbox = document.querySelector(`#${type}`);
...
function makeDownloadName() {
    ...
    const now = new Date();
    const friendlyDate = [now.getFullYear(), now.getMonth() + 1, now.getDate()].join('-');
    const name = [
        'sync-data-dump',
        friendlyDate,
        Date.now(),
...
function triggerDataDownload(nodesMap) {
    ...
    const anchor = document.querySelector('#dump-to-file-anchor');
    assert(anchor);
...
function createTypesCheckboxes(types) {
    const containerElt = document.querySelector('#node-type-checkboxes');
    assert(containerElt);
...
function onReceivedIncludeSpecificsInitialState(response) {
    const captureSpecifics = document.querySelector('#capture-specifics');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    requestListOfTypes();
    addWebUIListener('onReceivedIncludeSpecificsInitialState', onReceivedIncludeSpecificsInitialState);
    requestIncludeSpecificsInitialState();
});
const dumpToFileLink = document.querySelector('#dump-to-file');
