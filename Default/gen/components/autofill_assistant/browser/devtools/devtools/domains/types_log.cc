...
#include "components/autofill_assistant/browser/devtools/devtools/internal/type_conversions_log.h"
...
std::unique_ptr<LogEntry> LogEntry::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (source_value) {
    ...
    result->source_ = internal::FromValue<::autofill_assistant::log::LogEntrySource>::Parse(*source_value, errors);
  } else {
  ...
  if (level_value) {
    ...
    result->level_ = internal::FromValue<::autofill_assistant::log::LogEntryLevel>::Parse(*level_value, errors);
  } else {
  ...
  const base::Value* category_value = value.FindKey("category");
  if (category_value) {
    errors->SetName("category");
    result->category_ = internal::FromValue<::autofill_assistant::log::LogEntryCategory>::Parse(*category_value, errors);
  }
...
std::unique_ptr<base::Value> LogEntry::Serialize() const {
  ...
  if (category_)
    result->Set("category", internal::ToValue(category_.value()));
...
std::unique_ptr<ViolationSetting> ViolationSetting::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (name_value) {
    errors->SetName("name");
    result->name_ = internal::FromValue<::autofill_assistant::log::ViolationSettingName>::Parse(*name_value, errors);
  } else {
...
std::unique_ptr<StartViolationsReportParams> StartViolationsReportParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (config_value) {
    ...
    result->config_ = internal::FromValue<std::vector<std::unique_ptr<::autofill_assistant::log::ViolationSetting>>>::Parse(*config_value, errors);
  } else {
...
std::unique_ptr<EntryAddedParams> EntryAddedParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (entry_value) {
    ...
    result->entry_ = internal::FromValue<::autofill_assistant::log::LogEntry>::Parse(*entry_value, errors);
  } else {
