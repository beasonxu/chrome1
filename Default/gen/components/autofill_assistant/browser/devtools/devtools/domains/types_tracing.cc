...
#include "components/autofill_assistant/browser/devtools/devtools/internal/type_conversions_tracing.h"
...
std::unique_ptr<TraceConfig> TraceConfig::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (record_mode_value) {
    ...
    result->record_mode_ = internal::FromValue<::autofill_assistant::tracing::TraceConfigRecordMode>::Parse(*record_mode_value, errors);
  }
  const base::Value* trace_buffer_size_in_kb_value = value.FindKey("traceBufferSizeInKb");
...
std::unique_ptr<RequestMemoryDumpParams> RequestMemoryDumpParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  const base::Value* level_of_detail_value = value.FindKey("levelOfDetail");
  if (level_of_detail_value) {
    errors->SetName("levelOfDetail");
    result->level_of_detail_ = internal::FromValue<::autofill_assistant::tracing::MemoryDumpLevelOfDetail>::Parse(*level_of_detail_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
...
std::unique_ptr<StartParams> StartParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (transfer_mode_value) {
    ...
    result->transfer_mode_ = internal::FromValue<::autofill_assistant::tracing::StartTransferMode>::Parse(*transfer_mode_value, errors);
  }
  const base::Value* stream_format_value = value.FindKey("streamFormat");
  if (stream_format_value) {
    errors->SetName("streamFormat");
    result->stream_format_ = internal::FromValue<::autofill_assistant::tracing::StreamFormat>::Parse(*stream_format_value, errors);
  }
  ...
  if (stream_compression_value) {
    ...
    result->stream_compression_ = internal::FromValue<::autofill_assistant::tracing::StreamCompression>::Parse(*stream_compression_value, errors);
  }
  const base::Value* trace_config_value = value.FindKey("traceConfig");
  if (trace_config_value) {
    errors->SetName("traceConfig");
    result->trace_config_ = internal::FromValue<::autofill_assistant::tracing::TraceConfig>::Parse(*trace_config_value, errors);
  }
  const base::Value* perfetto_config_value = value.FindKey("perfettoConfig");
  if (perfetto_config_value) {
    errors->SetName("perfettoConfig");
    result->perfetto_config_ = internal::FromValue<std::string>::Parse(*perfetto_config_value, errors);
  }
  const base::Value* tracing_backend_value = value.FindKey("tracingBackend");
  if (tracing_backend_value) {
    errors->SetName("tracingBackend");
    result->tracing_backend_ = internal::FromValue<::autofill_assistant::tracing::TracingBackend>::Parse(*tracing_backend_value, errors);
  }
  errors->Pop();
  errors->Pop();
  if (errors->HasErrors())
...
std::unique_ptr<TracingCompleteParams> TracingCompleteParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (trace_format_value) {
    ...
    result->trace_format_ = internal::FromValue<::autofill_assistant::tracing::StreamFormat>::Parse(*trace_format_value, errors);
  }
  const base::Value* stream_compression_value = value.FindKey("streamCompression");
  if (stream_compression_value) {
    errors->SetName("streamCompression");
