...
#include "components/autofill_assistant/browser/devtools/devtools/internal/type_conversions_cast.h"
...
std::unique_ptr<base::Value> StartDesktopMirroringParams::Serialize() const {
  std::unique_ptr<base::DictionaryValue> result(new base::DictionaryValue());
  result->Set("sinkName", internal::ToValue(sink_name_));
  return std::move(result);
}
...
std::unique_ptr<base::Value> StartDesktopMirroringResult::Serialize() const {
  std::unique_ptr<base::DictionaryValue> result(new base::DictionaryValue());
  return std::move(result);
}
...
std::unique_ptr<SinksUpdatedParams> SinksUpdatedParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (sinks_value) {
    ...
    result->sinks_ = internal::FromValue<std::vector<std::unique_ptr<::autofill_assistant::cast::Sink>>>::Parse(*sinks_value, errors);
  } else {
