...
#include "components/autofill_assistant/browser/devtools/devtools/internal/type_conversions_background_service.h"
...
std::unique_ptr<BackgroundServiceEvent> BackgroundServiceEvent::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (service_value) {
    ...
    result->service_ = internal::FromValue<::autofill_assistant::background_service::ServiceName>::Parse(*service_value, errors);
  } else {
  ...
  if (event_metadata_value) {
    errors->SetName("eventMetadata");
    result->event_metadata_ = internal::FromValue<std::vector<std::unique_ptr<::autofill_assistant::background_service::EventMetadata>>>::Parse(*event_metadata_value, errors);
  } else {
...
std::unique_ptr<BackgroundServiceEventReceivedParams> BackgroundServiceEventReceivedParams::Parse(const base::Value& value, ErrorReporter* errors) {
  ...
  if (background_service_event_value) {
    errors->SetName("backgroundServiceEvent");
    result->background_service_event_ = internal::FromValue<::autofill_assistant::background_service::BackgroundServiceEvent>::Parse(*background_service_event_value, errors);
  } else {
