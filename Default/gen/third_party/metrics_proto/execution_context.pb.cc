...
static ::PROTOBUF_NAMESPACE_ID::internal::ExplicitlyConstructed<std::string> Thread_strings[22] = {};
...
static const ::PROTOBUF_NAMESPACE_ID::internal::EnumEntry Thread_entries[] = {
  ...
  { {Thread_names + 58, 9}, 7 },
  { {Thread_names + 67, 23}, 18 },
  { {Thread_names + 90, 11}, 2 },
  { {Thread_names + 101, 25}, 3 },
  { {Thread_names + 126, 15}, 8 },
  { {Thread_names + 141, 17}, 17 },
  { {Thread_names + 158, 9}, 6 },
  { {Thread_names + 167, 11}, 1 },
  { {Thread_names + 178, 12}, 20 },
  { {Thread_names + 190, 19}, 19 },
  { {Thread_names + 209, 12}, 12 },
  { {Thread_names + 221, 23}, 4 },
  { {Thread_names + 244, 13}, 9 },
  { {Thread_names + 257, 23}, 13 },
  { {Thread_names + 280, 21}, 15 },
  { {Thread_names + 301, 18}, 16 },
  { {Thread_names + 319, 14}, 0 },
  { {Thread_names + 333, 14}, 10 },
  { {Thread_names + 347, 13}, 21 },
};
...
static const int Thread_entries_by_number[] = {
  ...
  10, // 1 -> MAIN_THREAD
  5, // 2 -> FILE_THREAD
  6, // 3 -> FILE_USER_BLOCKING_THREAD
  14, // 4 -> PROCESS_LAUNCHER_THREAD
  0, // 5 -> CACHE_THREAD
  9, // 6 -> IO_THREAD
  3, // 7 -> DB_THREAD
  7, // 8 -> GPU_MAIN_THREAD
  15, // 9 -> RENDER_THREAD
  20, // 10 -> UTILITY_THREAD
  1, // 11 -> COMPOSITOR_THREAD
  13, // 12 -> OTHER_THREAD
  16, // 13 -> SCHEDULER_WORKER_THREAD
  2, // 14 -> COMPOSITOR_TILE_WORKER_THREAD
  17, // 15 -> SERVICE_WORKER_THREAD
  18, // 16 -> THREAD_POOL_THREAD
  8, // 17 -> GPU_MEMORY_THREAD
  4, // 18 -> DEDICATED_WORKER_THREAD
  12, // 19 -> MEMORY_INFRA_THREAD
  11, // 20 -> MEDIA_THREAD
  21, // 21 -> WEBRTC_THREAD
};
...
bool Thread_Parse(
  ...
  int int_value;
  bool success = ::PROTOBUF_NAMESPACE_ID::internal::LookUpEnumValue(
      Thread_entries, 22, name, &int_value);
  if (success) {
