...
void GetAllMatchedEntriesResult::set_entries(
  ...
  if (tag_ == Tag::kEntries) {
    *(data_.entries) = std::move(entries);
    ...
    DestroyActive();
    tag_ = Tag::kEntries;
    data_.entries = new std::vector<CacheEntryPtr>(
        std::move(entries));
  }
}
    ...
    [&](perfetto::TracedValue context){
      ...
      perfetto::WriteIntoTracedValueWithFallback(
           dict.AddItem("batch_operations"), in_batch_operations,
                        "<value of type std::vector<BatchOperationPtr>>");
      perfetto::WriteIntoTracedValueWithFallback(
           dict.AddItem("trace_id"), in_trace_id,
                        "<value of type int64_t>");
   });
...
void CacheStorageAsyncWaiter::Keys(
  ...
  proxy_->Keys(std::move(trace_id),
      base::BindOnce(
          [](base::RunLoop* loop,
             std::vector<::std::u16string>* out_keys
,
             const std::vector<::std::u16string>& keys) {*out_keys = std::move(keys);
            loop->Quit();
          },
          &loop,
          out_keys));
