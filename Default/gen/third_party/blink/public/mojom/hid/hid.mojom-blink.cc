...
DeviceIdFilter::DeviceIdFilter() : tag_(Tag::kVendor) {
...
UsageFilter::UsageFilter() : tag_(Tag::kPage) {
  data_.page = uint16_t();
}
...
uint32_t HidService::Forget_Sym::IPCStableHash() {
  ...
  // hash instead of __LINE__.
  constexpr uint32_t kHash = base::MD5Hash32Constexpr(
          "(Impl)blink::mojom::HidService::Forget");
  const uint32_t hash = kHash;
  base::debug::Alias(&hash);
  return hash;
}
...
void HidServiceProxy::Forget(
    ...
    [&](perfetto::TracedValue context){
      ...
      perfetto::WriteIntoTracedValueWithFallback(
           dict.AddItem("device_info"), in_device_info,
                        "<value of type ::device::mojom::blink::HidDeviceInfoPtr>");
   });
#endif
  const bool kExpectsResponse = true;
  const bool kIsSync = false;
  ...
  mojo::Message message(
      internal::kHidService_Forget_Name, kFlags, 0, 0, nullptr);
  mojo::internal::MessageFragment<
...
void HidServiceAsyncWaiter::Forget(
  ...
  base::RunLoop loop;
  proxy_->Forget(std::move(device_info),
      base::BindOnce(
          [](base::RunLoop* loop) {
            loop->Quit();
          },
