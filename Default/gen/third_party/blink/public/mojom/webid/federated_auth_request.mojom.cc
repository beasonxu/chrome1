...
class FederatedAuthRequest_RequestToken_ProxyToResponder : public ::mojo::internal::ProxyToResponder {
 ...
 private:
  ...
  void Run(
      RequestTokenStatus in_status, const absl::optional<std::string>& in_token);
};
...
,
             ...
             const absl::optional<std::string>& token) {*out_status = std::move(status);*out_token = std::move(token);
            loop->Quit();
          },
          &loop,
          out_status,
