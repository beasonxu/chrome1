...
void SharedWorkerConnectorInterceptorForTesting::Connect(::blink::mojom::SharedWorkerInfoPtr info, ::mojo::PendingRemote<::blink::mojom::SharedWorkerClient> client, ::blink::mojom::SharedWorkerCreationContextType creation_context_type, ::blink::MessagePortDescriptor message_port, ::mojo::PendingRemote<::blink::mojom::BlobURLToken> blob_url_token, int64_t client_ukm_source_id) {
  GetForwardingInterface()->Connect(std::move(info), std::move(client), std::move(creation_context_type), std::move(message_port), std::move(blob_url_token), std::move(client_ukm_source_id));
}
