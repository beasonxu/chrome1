...
(function() {
  ...
  WebPreferences.prototype.initDefaults_ = function() {
    ...
    this.forceEnableZoom = false;
    this.supportDeprecatedTargetDensityDpi = false;
    this.useLegacyBackgroundSizeShorthandBehavior = false;
    this.wideViewportQuirk = false;
    this.useWideViewport = false;
    this.forceZeroLayoutHeight = false;
    this.viewportMetaMergeContentQuirk = false;
    this.viewportMetaNonUserScalableQuirk = false;
    this.viewportMetaZeroValuesQuirk = false;
    this.clobberUserAgentInitialScaleQuirk = false;
    this.ignoreMainFrameOverflowHiddenQuirk = false;
    this.reportScreenSizeInPhysicalPixelsQuirk = false;
    this.reuseGlobalForUnownedMainFrame = false;
    this.spellcheckEnabledByDefault = false;
    this.videoFullscreenOrientationLockEnabled = false;
    this.videoRotateToFullscreenEnabled = false;
    this.embeddedMediaExperienceEnabled = false;
    this.cssHexAlphaColorEnabled = false;
    this.scrollTopLeftInteropEnabled = false;
    this.disableAcceleratedSmallCanvases = false;
    ...
    this.fontScaleFactor = 0;
    this.deviceScaleAdjustment = 0;
    this.defaultVideoPosterUrl = null;
    this.disableWebauthn = false;
  ...
  WebPreferences.validate = function(messageValidator, offset) {
    ...
    // validate WebPreferences.defaultVideoPosterUrl
    ...
    err = messageValidator.validateEnum(offset + codec.kStructHeaderSize + 244, AutoplayPolicy);
    if (err !== validator.validationError.NONE)
        return err;


    // validate WebPreferences.preferredColorScheme
    err = messageValidator.validateEnum(offset + codec.kStructHeaderSize + 248, preferred_color_scheme$.PreferredColorScheme);
    if (err !== validator.validationError.NONE)
        return err;


    // validate WebPreferences.preferredContrast
    err = messageValidator.validateEnum(offset + codec.kStructHeaderSize + 252, preferred_contrast$.PreferredContrast);
    if (err !== validator.validationError.NONE)
        return err;
    ...
    err = messageValidator.validateEnum(offset + codec.kStructHeaderSize + 260, EffectiveConnectionType);
    if (err !== validator.validationError.NONE)
        return err;
  ...
  WebPreferences.decode = function(decoder) {
    ...
    packed = decoder.readUint8();
    val.databasesEnabled = (packed >> 0) & 1 ? true : false;
    val.tabsToLinks = (packed >> 1) & 1 ? true : false;
    val.disableIpcFloodingProtection = (packed >> 2) & 1 ? true : false;
    val.hyperlinkAuditingEnabled = (packed >> 3) & 1 ? true : false;
    val.allowUniversalAccessFromFileUrls = (packed >> 4) & 1 ? true : false;
    val.allowFileAccessFromFileUrls = (packed >> 5) & 1 ? true : false;
    val.webgl1Enabled = (packed >> 6) & 1 ? true : false;
    val.webgl2Enabled = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.pepper3dEnabled = (packed >> 0) & 1 ? true : false;
    val.privilegedWebglExtensionsEnabled = (packed >> 1) & 1 ? true : false;
    val.webglErrorsToConsoleEnabled = (packed >> 2) & 1 ? true : false;
    val.hideScrollbars = (packed >> 3) & 1 ? true : false;
    val.enableWebkitScrollbarStyling = (packed >> 4) & 1 ? true : false;
    val.accelerated2dCanvasEnabled = (packed >> 5) & 1 ? true : false;
    val.canvas2dLayersEnabled = (packed >> 6) & 1 ? true : false;
    val.antialiased2dCanvasDisabled = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.antialiasedClips2dCanvasEnabled = (packed >> 0) & 1 ? true : false;
    val.acceleratedFiltersEnabled = (packed >> 1) & 1 ? true : false;
    val.deferredFiltersEnabled = (packed >> 2) & 1 ? true : false;
    val.containerCullingEnabled = (packed >> 3) & 1 ? true : false;
    val.allowRunningInsecureContent = (packed >> 4) & 1 ? true : false;
    val.disableReadingFromCanvas = (packed >> 5) & 1 ? true : false;
    val.strictMixedContentChecking = (packed >> 6) & 1 ? true : false;
    val.strictPowerfulFeatureRestrictions = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.allowGeolocationOnInsecureOrigins = (packed >> 0) & 1 ? true : false;
    val.strictlyBlockBlockableMixedContent = (packed >> 1) & 1 ? true : false;
    val.blockMixedPluginContent = (packed >> 2) & 1 ? true : false;
    val.passwordEchoEnabled = (packed >> 3) & 1 ? true : false;
    val.shouldClearDocumentBackground = (packed >> 4) & 1 ? true : false;
    val.enableScrollAnimator = (packed >> 5) & 1 ? true : false;
    val.threadedScrollingEnabled = (packed >> 6) & 1 ? true : false;
    val.prefersReducedMotion = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    ...
    packed = decoder.readUint8();
    val.navigateOnDragDrop = (packed >> 0) & 1 ? true : false;
    val.fakeNoAllocDirectCallForTestingEnabled = (packed >> 1) & 1 ? true : false;
    val.recordWholeDocument = (packed >> 2) & 1 ? true : false;
    val.stylusHandwritingEnabled = (packed >> 3) & 1 ? true : false;
    ...
    val.forceEnableZoom = (packed >> 4) & 1 ? true : false;
    val.supportDeprecatedTargetDensityDpi = (packed >> 5) & 1 ? true : false;
    val.useLegacyBackgroundSizeShorthandBehavior = (packed >> 6) & 1 ? true : false;
    val.wideViewportQuirk = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.useWideViewport = (packed >> 0) & 1 ? true : false;
    val.forceZeroLayoutHeight = (packed >> 1) & 1 ? true : false;
    val.viewportMetaMergeContentQuirk = (packed >> 2) & 1 ? true : false;
    val.viewportMetaNonUserScalableQuirk = (packed >> 3) & 1 ? true : false;
    val.viewportMetaZeroValuesQuirk = (packed >> 4) & 1 ? true : false;
    val.clobberUserAgentInitialScaleQuirk = (packed >> 5) & 1 ? true : false;
    val.ignoreMainFrameOverflowHiddenQuirk = (packed >> 6) & 1 ? true : false;
    val.reportScreenSizeInPhysicalPixelsQuirk = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.reuseGlobalForUnownedMainFrame = (packed >> 0) & 1 ? true : false;
    val.spellcheckEnabledByDefault = (packed >> 1) & 1 ? true : false;
    val.videoFullscreenOrientationLockEnabled = (packed >> 2) & 1 ? true : false;
    val.videoRotateToFullscreenEnabled = (packed >> 3) & 1 ? true : false;
    val.embeddedMediaExperienceEnabled = (packed >> 4) & 1 ? true : false;
    val.cssHexAlphaColorEnabled = (packed >> 5) & 1 ? true : false;
    val.scrollTopLeftInteropEnabled = (packed >> 6) & 1 ? true : false;
    val.disableAcceleratedSmallCanvases = (packed >> 7) & 1 ? true : false;
    val.v8CacheOptions =
    ...
    val.deviceScaleAdjustment =
        decoder.decodeStruct(codec.Float);
    val.defaultVideoPosterUrl =
        decoder.decodeStructPointer(url$.Url);
    packed = decoder.readUint8();
    val.disableWebauthn = (packed >> 0) & 1 ? true : false;
    ...
    val.pictureInPictureEnabled = (packed >> 6) & 1 ? true : false;
    val.translateServiceAvailable = (packed >> 7) & 1 ? true : false;
    packed = decoder.readUint8();
    val.lazyLoadEnabled = (packed >> 0) & 1 ? true : false;
    val.allowMixedContentUpgrades = (packed >> 1) & 1 ? true : false;
    val.alwaysShowFocus = (packed >> 2) & 1 ? true : false;
    val.touchDragDropEnabled = (packed >> 3) & 1 ? true : false;
    val.webxrImmersiveArAllowed = (packed >> 4) & 1 ? true : false;
  ...
  WebPreferences.encode = function(encoder, val) {
    ...
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.databasesEnabled & 1) << 0
    packed |= (val.tabsToLinks & 1) << 1
    packed |= (val.disableIpcFloodingProtection & 1) << 2
    packed |= (val.hyperlinkAuditingEnabled & 1) << 3
    packed |= (val.allowUniversalAccessFromFileUrls & 1) << 4
    packed |= (val.allowFileAccessFromFileUrls & 1) << 5
    packed |= (val.webgl1Enabled & 1) << 6
    packed |= (val.webgl2Enabled & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.pepper3dEnabled & 1) << 0
    packed |= (val.privilegedWebglExtensionsEnabled & 1) << 1
    packed |= (val.webglErrorsToConsoleEnabled & 1) << 2
    packed |= (val.hideScrollbars & 1) << 3
    packed |= (val.enableWebkitScrollbarStyling & 1) << 4
    packed |= (val.accelerated2dCanvasEnabled & 1) << 5
    packed |= (val.canvas2dLayersEnabled & 1) << 6
    packed |= (val.antialiased2dCanvasDisabled & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.antialiasedClips2dCanvasEnabled & 1) << 0
    packed |= (val.acceleratedFiltersEnabled & 1) << 1
    packed |= (val.deferredFiltersEnabled & 1) << 2
    packed |= (val.containerCullingEnabled & 1) << 3
    packed |= (val.allowRunningInsecureContent & 1) << 4
    packed |= (val.disableReadingFromCanvas & 1) << 5
    packed |= (val.strictMixedContentChecking & 1) << 6
    packed |= (val.strictPowerfulFeatureRestrictions & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.allowGeolocationOnInsecureOrigins & 1) << 0
    packed |= (val.strictlyBlockBlockableMixedContent & 1) << 1
    packed |= (val.blockMixedPluginContent & 1) << 2
    packed |= (val.passwordEchoEnabled & 1) << 3
    packed |= (val.shouldClearDocumentBackground & 1) << 4
    packed |= (val.enableScrollAnimator & 1) << 5
    packed |= (val.threadedScrollingEnabled & 1) << 6
    packed |= (val.prefersReducedMotion & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    ...
    packed = 0;
    packed |= (val.navigateOnDragDrop & 1) << 0
    packed |= (val.fakeNoAllocDirectCallForTestingEnabled & 1) << 1
    packed |= (val.recordWholeDocument & 1) << 2
    packed |= (val.stylusHandwritingEnabled & 1) << 3
    ...
    packed |= (val.forceEnableZoom & 1) << 4
    packed |= (val.supportDeprecatedTargetDensityDpi & 1) << 5
    packed |= (val.useLegacyBackgroundSizeShorthandBehavior & 1) << 6
    packed |= (val.wideViewportQuirk & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.useWideViewport & 1) << 0
    packed |= (val.forceZeroLayoutHeight & 1) << 1
    packed |= (val.viewportMetaMergeContentQuirk & 1) << 2
    packed |= (val.viewportMetaNonUserScalableQuirk & 1) << 3
    packed |= (val.viewportMetaZeroValuesQuirk & 1) << 4
    packed |= (val.clobberUserAgentInitialScaleQuirk & 1) << 5
    packed |= (val.ignoreMainFrameOverflowHiddenQuirk & 1) << 6
    packed |= (val.reportScreenSizeInPhysicalPixelsQuirk & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.reuseGlobalForUnownedMainFrame & 1) << 0
    packed |= (val.spellcheckEnabledByDefault & 1) << 1
    packed |= (val.videoFullscreenOrientationLockEnabled & 1) << 2
    packed |= (val.videoRotateToFullscreenEnabled & 1) << 3
    packed |= (val.embeddedMediaExperienceEnabled & 1) << 4
    packed |= (val.cssHexAlphaColorEnabled & 1) << 5
    packed |= (val.scrollTopLeftInteropEnabled & 1) << 6
    packed |= (val.disableAcceleratedSmallCanvases & 1) << 7
    encoder.writeUint8(packed);
    ...
    encoder.encodeStruct(codec.Float, val.fontScaleFactor);
    encoder.encodeStruct(codec.Float, val.deviceScaleAdjustment);
    encoder.encodeStructPointer(url$.Url, val.defaultVideoPosterUrl);
    packed = 0;
    packed |= (val.disableWebauthn & 1) << 0
    ...
    packed |= (val.pictureInPictureEnabled & 1) << 6
    packed |= (val.translateServiceAvailable & 1) << 7
    encoder.writeUint8(packed);
    packed = 0;
    packed |= (val.lazyLoadEnabled & 1) << 0
    packed |= (val.allowMixedContentUpgrades & 1) << 1
    packed |= (val.alwaysShowFocus & 1) << 2
    packed |= (val.touchDragDropEnabled & 1) << 3
    packed |= (val.webxrImmersiveArAllowed & 1) << 4
