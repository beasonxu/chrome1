...
const DocumentPolicyFeatureInfoMap& GetDocumentPolicyFeatureInfoMap() {
  static const base::NoDestructor<DocumentPolicyFeatureInfoMap> feature_info_map([] {
    DocumentPolicyFeatureInfoMap map({        {
          ...
          mojom::DocumentPolicyFeature::kJSProfiling,
          {
            "js-profiling",
            PolicyValue::CreateBool(false)
          }
        },        {
          mojom::DocumentPolicyFeature::kSyncXHR,
          {
            "sync-xhr",
            PolicyValue::CreateBool(true)
          }
        },        {
          mojom::DocumentPolicyFeature::kDocumentDomain,
          {
            "document-domain",
            PolicyValue::CreateBool(true)
          }
        },    });        if (base::FeatureList::IsEnabled(features::kDisableDocumentDomainByDefault)) {
          map.find(mojom::DocumentPolicyFeature::kDocumentDomain)->second.default_value =
            PolicyValue::CreateBool(false);
        }
    return map;
  }());
