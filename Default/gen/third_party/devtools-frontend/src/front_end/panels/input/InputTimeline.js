...
export class InputTimeline extends UI.Widget.VBox {
    ...
    isAvailableState() {
        return this.state === "Idle" /* State.Idle */ || this.state === "ReplayPaused" /* State.ReplayPaused */;
    }
    updateControls() {
        this.toggleRecordAction.setToggled(this.state === "Recording" /* State.Recording */);
        this.toggleRecordAction.setEnabled(this.isAvailableState() || this.state === "Recording" /* State.Recording */);
        this.startReplayAction.setEnabled(this.isAvailableState() && Boolean(this.tracingModel));
        this.togglePauseAction.setEnabled(this.state === "Replaying" /* State.Replaying */ || this.state === "ReplayPaused" /* State.ReplayPaused */);
        this.togglePauseAction.setToggled(this.state === "ReplayPaused" /* State.ReplayPaused */);
    ...
    toggleReplayPause() {
        switch (this.state) {
            case "Replaying" /* State.Replaying */: {
                this.pauseReplay();
                break;
            }
            case "ReplayPaused" /* State.ReplayPaused */: {
                this.resumeReplay();
                break;
            }
        }
    ...
    async replayEvents() {
        ...
        this.setState("Replaying" /* State.Replaying */);
    ...
    pauseReplay() {
        ...
        this.setState("ReplayPaused" /* State.ReplayPaused */);
    }
    resumeReplay() {
        if (!this.inputModel) {
            return;
