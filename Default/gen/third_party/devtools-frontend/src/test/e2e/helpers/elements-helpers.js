...
const clickTreeElementWithPartialText = async (text) => {
    ...
    if (handle) {
        await (0, helper_js_1.click)(handle);
        return true;
    }
    throw false;
};
...
const filterComputedProperties = async (filterString) => {
    ...
    const filterInput = await (0, helper_js_1.waitFor)('[aria-label="Filter Computed Styles"]', computedPanel);
    await (0, helper_js_1.click)(filterInput);
