...
describeWithLocale('LinearMemoryInspector', () => {
    ...
    function setUpComponent() {
        ...
        const data = {
            ...
            endianness: "Little Endian" /* LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.Endianness.Little */,
    ...
    it('can change endianness settings on event', () => {
        ...
        assert.deepEqual(select.value, "Little Endian" /* LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.Endianness.Little */);
        const endianSetting = "Big Endian" /* LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.Endianness.Big */;
    ...
    it('updates current address if user triggers a jumptopointeraddress event', () => {
        const { component, data } = setUpComponent();
        data.valueTypes = new Set(["Pointer 32-bit" /* LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.ValueType.Pointer32 */]);
        data.memory = new Uint8Array([2, 0, 0, 0]);
        data.outerMemoryLength = data.memory.length;
        data.address = 0;
        data.endianness = "Little Endian" /* LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.Endianness.Little */;
        component.data = data;
        const interpreter = getValueInterpreter(component);
    ...
    it('leaves the navigator address as inputted by user on edit event', () => {
        const { component } = setUpComponent();
        const navigator = getNavigator(component);
        triggerAddressChangedEvent(component, '2', "Edit" /* LinearMemoryInspectorModule.LinearMemoryNavigator.Mode.Edit */);
        assertUpdatesInNavigator(navigator, '2', 'Enter address');
    });
    it('changes navigator address (to hex) on valid user submit event', () => {
        const { component } = setUpComponent();
        const navigator = getNavigator(component);
        triggerAddressChangedEvent(component, '2', "Submitted" /* LinearMemoryInspectorModule.LinearMemoryNavigator.Mode.Submitted */);
    ...
    it('triggers event on settings changed when value type is changed', async () => {
        ...
        const valueType = "Integer 16-bit" /* LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.ValueType.Int16 */;
    ...
    it('triggers event on settings changed when value type mode is changed', async () => {
        ...
        const eventPromise = getEventPromise(component, 'settingschanged');
        const valueType = "Integer 16-bit" /* LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.ValueType.Int16 */;
        const valueTypeMode = "hex" /* LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.ValueTypeMode.Hexadecimal */;
    ...
    it('triggers event on settings changed when endianness is changed', async () => {
        ...
        const interpreter = getValueInterpreter(component);
        const eventPromise = getEventPromise(component, 'settingschanged');
        const endianness = "Big Endian" /* LinearMemoryInspectorModule.ValueInterpreterDisplayUtils.Endianness.Big */;
