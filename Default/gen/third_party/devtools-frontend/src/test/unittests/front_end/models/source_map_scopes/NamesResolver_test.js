...
class MockProtocolBackend {
    ...
    async addScript(target, script, sourceMap) {
    ...
    createProtocolScope(type, object, scriptId, startColumn, endColumn) {
        return {
            type,
            object,
...
describeWithMockConnection('NameResolver', () => {
    ...
    function parseScopeChain(scopeDescriptor) {
        ...
        const scopeChain = [{ type: "local" /* Protocol.Debugger.ScopeType.Local */, startColumn: functionStart, endColumn: functionEnd + 1 }];
        ...
        if (blockScopeStart >= 0) {
            ...
            scopeChain.unshift({ type: "block" /* Protocol.Debugger.ScopeType.Block */, startColumn: blockScopeStart, endColumn: blockScopeEnd + 1 });
        }
        return scopeChain;
    }
    async function initializeModelAndScopes(script, scopeDescriptor, sourceMap, scopeObject = null) {
        const debuggerModel = target.model(SDK.DebuggerModel.DebuggerModel);
        const scriptObject = await backend.addScript(target, script, sourceMap);
        const parsedScopes = parseScopeChain(scopeDescriptor);
        const scopeChain = parsedScopes.map(s => backend.createProtocolScope(s.type, { type: "object" /* Protocol.Runtime.RemoteObjectType.Object */ }, scriptObject.scriptId, s.startColumn, s.endColumn));
    ...
    for (const test of tests) {
        it(test.name, async () => {
            const { functionScope, scope } = await initializeModelAndScopes({ url: URL, content: test.source }, test.scopes, { url: 'file:///dummy.map', content: dummyMapContent });
    ...
    it('resolves name tokens merged with commas (without source map names)', async () => {
        ...
        const { scope } = await initializeModelAndScopes({ url: URL, content: source }, scopes, { url: sourceMapUrl, content: sourceMapContent }, scopeObject);
...
//# sourceMappingURL=NamesResolver_test.js.map