...
import { assertElement, assertShadowRoot, getElementWithinComponent, getEventPromise, renderElementIntoDOM, } from '../../../helpers/DOMHelpers.js';
...
describeWithLocale('LinearMemoryInspectorHighlightChipList', () => {
    ...
    it('sends event when clicking on jump to highlighted memory', async () => {
        const eventPromise = getEventPromise(component, LinearMemoryInspector.LinearMemoryHighlightChipList.JumpToHighlightedMemoryEvent.eventName);
        const shadowRoot = component.shadowRoot;
        assertShadowRoot(shadowRoot);
        const button = shadowRoot.querySelector(HIGHLIGHT_PILL_JUMP_BUTTON_SELECTOR);
        assertElement(button, HTMLButtonElement);
    ...
    it('sends event when clicking on delete highlight chip', async () => {
        const eventPromise = getEventPromise(component, LinearMemoryInspector.LinearMemoryHighlightChipList.DeleteMemoryHighlightEvent.eventName);
        const shadowRoot = component.shadowRoot;
        assertShadowRoot(shadowRoot);
...
//# sourceMappingURL=LinearMemoryInspectorHighlightChipList_test.js.map