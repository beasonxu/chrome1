...
describeWithLocale('ValueInterpreterDisplay', () => {
    const combinationsForNumbers = [
        { endianness: "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */, signed: true },
        { endianness: "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */, signed: false },
        { endianness: "Big Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Big */, signed: false },
        { endianness: "Big Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Big */, signed: true },
    ];
    function testNumberFormatCombinations(baseData, combinations) {
        ...
        for (let i = 0; i < combinations.length; ++i) {
            ...
            let expectedValue;
            const isLittleEndian = endianness === "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */;
            const view = new DataView(baseData.buffer);
            switch (baseData.type) {
                case "Integer 8-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int8 */:
                    expectedValue = signed ? -expectedIntValue : expectedIntValue;
                    signed ? view.setInt8(0, expectedValue) : view.setInt8(0, expectedValue);
                    break;
                case "Integer 16-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int16 */:
                    expectedValue = signed ? -expectedIntValue : expectedIntValue;
                    signed ? view.setInt16(0, expectedValue, isLittleEndian) : view.setUint16(0, expectedValue, isLittleEndian);
                    break;
                case "Integer 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int32 */:
                    expectedValue = signed ? -expectedIntValue : expectedIntValue;
                    signed ? view.setInt32(0, expectedValue, isLittleEndian) : view.setUint32(0, expectedValue, isLittleEndian);
                    break;
                case "Integer 64-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int64 */:
                    expectedValue = signed ? -expectedIntValue : expectedIntValue;
                    signed ? view.setBigInt64(0, BigInt(expectedValue), isLittleEndian) :
                        view.setBigUint64(0, BigInt(expectedValue), isLittleEndian);
                    break;
                case "Float 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Float32 */:
                    expectedValue = expectedFloatValue;
                    view.setFloat32(0, expectedValue, isLittleEndian);
                    break;
                case "Float 64-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Float64 */:
                    expectedValue = expectedFloatValue;
                    view.setFloat64(0, expectedValue, isLittleEndian);
                    break;
                case "Pointer 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Pointer32 */:
                    expectedValue = '0x' + expectedIntValue.toString(16);
                    view.setInt32(0, expectedIntValue, isLittleEndian);
                    break;
                case "Pointer 64-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Pointer64 */:
                    expectedValue = '0x' + expectedIntValue.toString(16);
    ...
    it('correctly formats signed/unsigned and endianness for Integer 8-bit (decimal)', () => {
        const formatData = {
            ...
            type: "Integer 8-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int8 */,
            mode: "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */,
        };
    ...
    it('correctly formats signed/unsigned and endianness for Integer 16-bit (decimal)', () => {
        const formatData = {
            ...
            type: "Integer 16-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int16 */,
            mode: "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */,
        };
        testNumberFormatCombinations(formatData, combinationsForNumbers);
    });
    it('correctly formats signed/unsigned and endianness for Integer 32-bit (decimal)', () => {
        const formatData = {
            ...
            type: "Integer 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int32 */,
            mode: "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */,
        };
        testNumberFormatCombinations(formatData, combinationsForNumbers);
    });
    it('correctly formats signed/unsigned and endianness for Integer 64-bit (decimal)', () => {
        const formatData = {
            ...
            type: "Integer 64-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int64 */,
            mode: "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */,
        };
        testNumberFormatCombinations(formatData, combinationsForNumbers);
    });
    it('correctly formats endianness for Float 32-bit (decimal)', () => {
        const formatData = {
            buffer: new ArrayBuffer(4),
            type: "Float 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Float32 */,
            mode: "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */,
        };
        testNumberFormatCombinations(formatData, combinationsForNumbers);
    });
    it('correctly formats endianness for Float 64-bit (decimal)', () => {
        const formatData = {
            buffer: new ArrayBuffer(8),
            type: "Float 64-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Float64 */,
            mode: "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */,
        };
        testNumberFormatCombinations(formatData, combinationsForNumbers);
    });
    it('correctly formats endianness for Pointer 32-bit', () => {
        const formatData = {
            buffer: new ArrayBuffer(4),
            type: "Pointer 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Pointer32 */,
            mode: "hex" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Hexadecimal */,
        };
        testNumberFormatCombinations(formatData, combinationsForNumbers);
    });
    it('correctly formats endianness for Pointer 64-bit', () => {
        const formatData = {
            buffer: new ArrayBuffer(8),
            type: "Pointer 64-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Pointer64 */,
            mode: "hex" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Hexadecimal */,
        };
        testNumberFormatCombinations(formatData, combinationsForNumbers);
    });
    it('correctly formats floats in decimal mode', () => {
        ...
        const actualValue = LinearMemoryInspector.ValueInterpreterDisplayUtils.formatFloat(expectedFloat, "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */);
        assert.strictEqual(actualValue, '341.34');
    });
    it('correctly formats floats in scientific mode', () => {
        const expectedFloat = 341.34;
        const actualValue = LinearMemoryInspector.ValueInterpreterDisplayUtils.formatFloat(expectedFloat, "sci" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Scientific */);
        assert.strictEqual(actualValue, '3.41e+2');
    });
    it('correctly formats integers in decimal mode', () => {
        const expectedInteger = 120;
        const actualValue = LinearMemoryInspector.ValueInterpreterDisplayUtils.formatInteger(expectedInteger, "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */);
        assert.strictEqual(actualValue, '120');
    });
    it('correctly formats integers in hexadecimal mode', () => {
        const expectedInteger = 16;
        const actualValue = LinearMemoryInspector.ValueInterpreterDisplayUtils.formatInteger(expectedInteger, "hex" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Hexadecimal */);
        assert.strictEqual(actualValue, '0x10');
    });
    it('returns N/A for negative hex numbers', () => {
        const negativeInteger = -16;
        const actualValue = LinearMemoryInspector.ValueInterpreterDisplayUtils.formatInteger(negativeInteger, "hex" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Hexadecimal */);
        assert.strictEqual(actualValue, 'N/A');
    });
    it('correctly formats integers in octal mode', () => {
        const expectedInteger = 16;
        const actualValue = LinearMemoryInspector.ValueInterpreterDisplayUtils.formatInteger(expectedInteger, "oct" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Octal */);
    ...
    it('renders pointer values in LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypes', () => {
        ...
        component.data = {
            ...
            endianness: "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */,
            valueTypes: new Set([
                "Pointer 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Pointer32 */,
                "Pointer 64-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Pointer64 */,
            ]),
    ...
    it('renders value in selected LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypes', () => {
        ...
        component.data = {
            ...
            endianness: "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */,
            valueTypes: new Set([
                "Integer 16-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int16 */,
                "Float 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Float32 */,
            ]),
            memoryLength: array.length,
        };
        renderElementIntoDOM(component);
    ...
    it('renders only unsigned values for Octal and Hexadecimal representation', () => {
        ...
        component.data = {
            ...
            endianness: "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */,
            valueTypes: new Set([
                "Integer 8-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int8 */,
                "Integer 16-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int16 */,
                "Integer 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int32 */,
            ]),
            valueTypeModes: new Map([
                [
                    "Integer 8-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int8 */,
                    "oct" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Octal */,
                ],
                [
                    "Integer 16-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int16 */,
                    "hex" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Hexadecimal */,
                ],
                [
                    "Integer 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Int32 */,
                    "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */,
                ],
            ]),
            memoryLength: array.length,
        };
    ...
    it('triggers a value changed event on selecting a new mode', async () => {
        const component = new LinearMemoryInspector.ValueInterpreterDisplay.ValueInterpreterDisplay();
        const array = [1, 132, 172, 71];
        const oldMode = "dec" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Decimal */;
        const newMode = "sci" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueTypeMode.Scientific */;
        const mapping = LinearMemoryInspector.ValueInterpreterDisplayUtils.getDefaultValueTypeMapping();
        mapping.set("Float 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Float32 */, oldMode);
        component.data = {
            buffer: new Uint8Array(array).buffer,
            endianness: "Little Endian" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.Endianness.Little */,
            valueTypes: new Set([
        ...
        assert.deepEqual(event.data, { type: "Float 32-bit" /* LinearMemoryInspector.ValueInterpreterDisplayUtils.ValueType.Float32 */, mode: newMode });
    });
