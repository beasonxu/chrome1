...
export class DescriptorPanel {
    ...
    getButtonElementFromTemplate_() {
        const buttonTemplate = this.rootElement_.getRootNode()
            .querySelector('#raw-data-tree-button');
    ...
    renderIndexItem_(rawData, offset, item, fieldLabel) {
        ...
        if (index > 0) {
            ...
            if (!this.stringDescriptorPanel_.stringDescriptorIndexes.has(index)) {
                ...
                this.stringDescriptorPanel_.indexesListElement.appendChild(optionElement);
            ...
            button.addEventListener('click', (event) => {
                ...
                const children = item.shadowRoot.querySelector('.tree-children');
                assert(children);
                children.textContent = '';
                assert(this.stringDescriptorPanel_);
                this.stringDescriptorPanel_.clearView();
                this.stringDescriptorPanel_.getStringDescriptorForAllLanguages_(index, item);
            });
        }
        else if (index < 0) {
            // Delete the ': ' in fieldLabel.
            const fieldName = fieldLabel.slice(0, -2);
            showError(`Invalid String Descriptor occurs in field ${fieldName} of this descriptor.`, this.rootElement_);
        }
    }
    /**
     ...
     */
    renderUrlDescriptorIndexItem_(rawData, offset, item, _fieldLabel) {
        const index = rawData[offset];
        if (index > 0) {
            const button = this.getButtonElementFromTemplate_();
            item.labelElement.appendChild(button);
            button.addEventListener('click', (event) => {
                ...
                assert(children);
                children.textContent = '';
                this.getUrlDescriptor_(rawData, offset - WEB_USB_URL_DESCRIPTOR_INDEX_OFFSET, item);
            });
        }
    }
    /**
     ...
     */
    async renderMsOs20DescriptorVendorSpecific_(rawData, offset, item) {
        ...
        const msOs20DescriptorSetLength = data.getUint16(MS_OS_20_SET_TOTAL_LENGTH_OFFSET, true);
        const button = this.getButtonElementFromTemplate_();
        item.labelElement.appendChild(button);
        button.addEventListener('click', async (event) => {
            ...
            this.rootElement_.querySelectorAll('descriptorpaneltitle')
                .forEach(el => el.remove());
            const msOs20RawData = await this.getMsOs20DescriptorSet_(vendorCode, msOs20DescriptorSetLength);
     ...
     *     descriptor set information.
     */
    async renderMsOs20DescriptorSetAltEnum_(rawData, offset, item) {
        ...
        if (altEnumCode !== 0) {
            ...
            button.addEventListener('click', async (event) => {
                event.stopPropagation();
                await this.sendMsOs20DescriptorSetAltEnumCommand_(vendorCode, altEnumCode);
            });
        }
    }
    /**
     ...
     *     descriptor.
     */
    renderFeatureRegistryPropertyDataItem_(rawData, offset, item, _fieldLabel, featureRegistryPropertyDataType, length) {
        let data;
        switch (featureRegistryPropertyDataType) {
            ...
            case 0:
            default:
                item.label += `Illegal Data Type. (${featureRegistryPropertyDataType} should be reserved.)`;
                break;
        }
    }
    /**
     ...
     * and raw form view.
     */
    async renderStandardDescriptor_(data, languageCode = 0, treeItem) {
        ...
        while (offset + STANDARD_DESCRIPTOR_TYPE_OFFSET < data.length) {
            ...
            switch (descriptorType) {
                case DEVICE_DESCRIPTOR_TYPE:
                    this.renderDeviceDescriptor_(rawDataTreeRoot, rawDataByteElement, data, offset);
                    break;
                case CONFIGURATION_DESCRIPTOR_TYPE:
                    ...
                    this.renderConfigurationDescriptor_(rawDataTreeRoot, rawDataByteElement, data, offset);
                    break;
                case STRING_DESCRIPTOR_TYPE:
                    this.renderStringDescriptorForLanguageCode_(rawDataTreeRoot, rawDataByteElement, data, offset, languageCode, treeItem);
                    break;
                case INTERFACE_DESCRIPTOR_TYPE:
                    ...
                    lastInterfaceItem = this.renderInterfaceDescriptor_(rawDataTreeRoot, rawDataByteElement, data, offset, indexInterface);
                    indexInterface++;
                    break;
                case ENDPOINT_DESCRIPTOR_TYPE:
                    const treeRoot = lastInterfaceItem || rawDataTreeRoot;
                    this.renderEndpointDescriptor_(treeRoot, rawDataByteElement, data, offset, indexEndpoint);
                    indexEndpoint++;
                    break;
                case BOS_DESCRIPTOR_TYPE:
                    this.renderBosDescriptor_(rawDataTreeRoot, rawDataByteElement, data, offset);
                    expectNumDevCapabilities =
                        data[BOS_DESCRIPTOR_NUM_DEVICE_CAPABILITIES_OFFSET];
                    break;
                case DEVICE_CAPABILITY_DESCRIPTOR_TYPE:
                    await this.renderDeviceCapabilityDescriptor_(rawDataTreeRoot, rawDataByteElement, data, offset, indexDevCapability);
                    indexDevCapability++;
                    break;
                default:
                    this.renderUnknownDescriptor_(rawDataTreeRoot, rawDataByteElement, data, offset, indexUnknown);
                    indexUnknown++;
                    break;
            }
            offset += length;
        ...
        if (expectNumInterfaces !== indexInterface) {
            showError(`Expected to find ${expectNumInterfaces} interface descriptors ` +
                `but only encountered ${indexInterface}.`, this.rootElement_);
        }
        if (expectNumEndpoints !== indexEndpoint) {
            showError(`Expected to find ${expectNumEndpoints} interface descriptors ` +
                `but only encountered ${indexEndpoint}.`, this.rootElement_);
        }
        if (expectNumDevCapabilities !== indexDevCapability) {
            showError(`Expected to find ${expectNumDevCapabilities} ` +
                `device capability descriptors but only encountered ${indexDevCapability}.`, this.rootElement_);
        }
    ...
    async getDeviceDescriptor() {
        ...
        try {
            ...
            const response = await this.usbDeviceProxy_.controlTransferIn(usbControlTransferParams, DEVICE_DESCRIPTOR_LENGTH, CONTROL_TRANSFER_TIMEOUT_MS);
            checkTransferSuccess(response.status, 'Failed to read the device descriptor.', this.rootElement_);
     ...
     */
    async renderDeviceDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset) {
        const fields = [
            {
        ...
        renderRawDataTree(rawDataTreeRoot, rawDataByteElement, fields, rawData, offset, this.rootElement_);
        document.body.dispatchEvent(new CustomEvent('device-descriptor-complete-for-test', { bubbles: true, composed: true }));
    }
    /**
    ...
    async getConfigurationDescriptor() {
        ...
        try {
            await this.usbDeviceProxy_.open();
            let response = await this.usbDeviceProxy_.controlTransferIn(usbControlTransferParams, CONFIGURATION_DESCRIPTOR_LENGTH, CONTROL_TRANSFER_TIMEOUT_MS);
            checkTransferSuccess(response.status, 'Failed to read the device configuration descriptor to determine ' +
                'the total descriptor length.', this.rootElement_);
            const dataView = new DataView(new Uint8Array(response.data.buffer).buffer);
            const length = dataView.getUint16(CONFIGURATION_DESCRIPTOR_TOTAL_LENGTH_OFFSET, true);
            // Re-gets the data using the full length.
            response = await this.usbDeviceProxy_.controlTransferIn(usbControlTransferParams, length, CONTROL_TRANSFER_TIMEOUT_MS);
            checkTransferSuccess(response.status, 'Failed to read the complete configuration descriptor.', this.rootElement_);
            this.renderStandardDescriptor_(new Uint8Array(response.data.buffer));
        }
        catch (e) {
            showError(e.message, this.rootElement_);
     ...
     */
    async renderConfigurationDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset) {
        const fields = [
            {
     ...
     */
    renderInterfaceDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexInterface) {
        const parentClassName = `descriptor-interface-${indexInterface}`;
        const interfaceItem = customTreeItem(`Interface ${indexInterface}`, parentClassName);
        ...
        renderRawDataTree(interfaceItem, rawDataByteElement, fields, rawData, offset, this.rootElement_, parentClassName);
     ...
     *     descriptor.
     */
    renderEndpointDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexEndpoint) {
        const parentClassName = `descriptor-endpoint-${indexEndpoint}`;
        const endpointItem = customTreeItem(`Endpoint ${indexEndpoint}`, parentClassName);
        ...
        renderRawDataTree(endpointItem, rawDataByteElement, fields, rawData, offset, this.rootElement_, parentClassName);
    }
    /**
     ...
     */
    renderUnknownDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexUnknown) {
        ...
        const unknownItem = customTreeItem(`Unknown Descriptor ${indexUnknown}`, parentClassName);
        ...
        let offset = renderRawDataTree(unknownItem, rawDataByteElement, fields, rawData, originalOffset, this.rootElement_, parentClassName);
    ...
    async getAllLanguageCodes() {
        ...
        try {
            ...
            checkTransferSuccess(response.status, 'Failed to read the device string descriptor to determine ' +
                'all supported languages.', this.rootElement_);
        }
        catch (e) {
            showError(e.message, this.rootElement_);
     ...
     */
    async getStringDescriptorForLanguageCode_(index, languageCode, treeItem) {
        const usbControlTransferParams = {
            type: UsbControlTransferType.STANDARD,
            recipient: UsbControlTransferRecipient.DEVICE,
            request: GET_DESCRIPTOR_REQUEST,
        ...
        try {
            await this.usbDeviceProxy_.open();
            const response = await this.usbDeviceProxy_.controlTransferIn(usbControlTransferParams, MAX_STRING_DESCRIPTOR_LENGTH, CONTROL_TRANSFER_TIMEOUT_MS);
            checkTransferSuccess(response.status, `Failed to read the device string descriptor of index: ${index}, language: ${parseLanguageCode(languageCode)}.`, this.rootElement_);
            assert(this.indexInput_);
            this.indexInput_.value = index.toString();
            this.renderStandardDescriptor_(new Uint8Array(response.data.buffer), languageCode, treeItem);
        }
        catch (e) {
            showError(e.message, this.rootElement_);
        }
     ...
     */
    renderStringDescriptorForLanguageCode_(rawDataTreeRoot, rawDataByteElement, rawData, offset, languageCode = 0, treeItem) {
        ...
        const stringDescriptorItem = customTreeItem(`${languageStr}: ${stringDescriptor}`, parentClassName);
        ...
        renderRawDataTree(stringDescriptorItem, rawDataByteElement, fields, rawData, offset, this.rootElement_, parentClassName);
    }
    /**
     * Gets string descriptor in all supported languages of current device with
     * given index.
     */
    async getStringDescriptorForAllLanguages_(index, treeItem) {
        this.rootElement_.hidden = false;
        assert(this.indexInput_);
        this.indexInput_.value = index.toString();
        ...
        for (const languageCode of languageCodesList) {
            await this.getStringDescriptorForLanguageCode_(index, languageCode, treeItem);
        }
    }
    /**
    ...
    initialStringDescriptorPanel(tabId) {
        ...
        this.rootElement_.querySelectorAll('input').forEach(el => el.setAttribute('list', `${el.getAttribute('list')}-${tabId}`));
        ...
        assert(button);
        const indexInput = this.rootElement_.querySelector('#index-input');
        this.indexInput_ = indexInput;
        const languageCodeInput = this.rootElement_.querySelector('#language-code-input');
    ...
    async getBosDescriptor() {
        ...
        try {
            await this.usbDeviceProxy_.open();
            let response = await this.usbDeviceProxy_.controlTransferIn(usbControlTransferParams, BOS_DESCRIPTOR_HEADER_LENGTH, CONTROL_TRANSFER_TIMEOUT_MS);
            checkTransferSuccess(response.status, 'Failed to read the device BOS descriptor to determine ' +
                'the total descriptor length.', this.rootElement_);
            const dataView = new DataView(new Uint8Array(response.data.buffer).buffer);
            const length = dataView.getUint16(BOS_DESCRIPTOR_TOTAL_LENGTH_OFFSET, true);
            // Re-gets the data using the full length.
            response = await this.usbDeviceProxy_.controlTransferIn(usbControlTransferParams, length, CONTROL_TRANSFER_TIMEOUT_MS);
            checkTransferSuccess(response.status, 'Failed to read the complete BOS descriptor.', this.rootElement_);
            await this.renderStandardDescriptor_(new Uint8Array(response.data.buffer));
        }
        catch (e) {
            showError(e.message, this.rootElement_);
        }
     ...
     * @param offset The start offset of the BOS descriptor.
     */
    async renderDeviceCapabilityDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexDevCapability) {
        switch (rawData[offset + BOS_DESCRIPTOR_DEVICE_CAPABILITY_TYPE_OFFSET]) {
            case DEVICE_CAPABILITY_DESCRIPTOR_TYPE_PLATFORM_TYPE:
                if (isSameUuid(rawData, offset, WEB_USB_CAPABILITY_UUID)) {
                    this.renderWebUsbPlatformDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexDevCapability);
                    break;
                }
                else if (isSameUuid(rawData, offset, MS_OS_20_PLATFORM_CAPABILITY_UUID)) {
                    this.renderMsOs20PlatformDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexDevCapability);
                    break;
                }
                else {
                    this.renderUnknownBosDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexDevCapability);
                    break;
                }
            default:
                this.renderUnknownBosDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexDevCapability);
     ...
     */
    renderWebUsbPlatformDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexWebUsb) {
        ...
        renderRawDataTree(webUsbItem, rawDataByteElement, fields, rawData, offset, this.rootElement_, parentClassName);
    }
    /**
     ...
     *     capability descriptor.
     */
    renderMsOs20PlatformDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexMsOs20) {
        const parentClassName = `descriptor-ms-os-20-${indexMsOs20}`;
        const msOs20Item = customTreeItem(`Microsoft OS 2.0 Descriptor`, parentClassName);
        ...
        offset = renderRawDataTree(msOs20Item, rawDataByteElement, fields, rawData, offset, this.rootElement_, parentClassName);
        ...
        while (offset < rawData.length) {
            offset = this.renderMsOs20DescriptorSetInfo_(msOs20Item, rawDataByteElement, rawData, offset, indexMsOs20DescriptorSetInfo, indexMsOs20);
     ...
     */
    renderMsOs20DescriptorSetInfo_(rawDataTreeRoot, rawDataByteElement, rawData, offset, indexMsOs20DescriptorSetInfo, indexMsOs20) {
        const parentClassName = `descriptor-ms-os-20-set-info-${indexMsOs20DescriptorSetInfo}`;
        const msOs20SetInfoItem = customTreeItem(`Microsoft OS 2.0 Descriptor Set Information`, parentClassName);
        ...
        const fields = [
            ...
            },
            {
                ...
                size: 1,
                formatter: formatByte,
                extraTreeItemFormatter: (rawData, offset, item, _fieldLabel) => this.renderMsOs20DescriptorVendorSpecific_(rawData, offset - MS_OS_20_VENDOR_CODE_ITEM_OFFSET, item),
            },
            {
                label: 'Alternate Enumeration Code: ',
                size: 1,
                formatter: formatByte,
                extraTreeItemFormatter: (rawData, offset, item, _fieldLabel) => this.renderMsOs20DescriptorSetAltEnum_(rawData, offset - MS_OS_20_ALT_ENUM_CODE_ITEM_OFFSET, item),
            },
        ];
        return renderRawDataTree(msOs20SetInfoItem, rawDataByteElement, fields, rawData, offset, this.rootElement_, parentClassName, `descriptor-ms-os-20-${indexMsOs20}`);
    }
    /**
     ...
     *     capability descriptor.
     */
    renderUnknownBosDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexUnknownDevCapability) {
        const length = rawData[originalOffset + STANDARD_DESCRIPTOR_LENGTH_OFFSET];
        const parentClassName = `descriptor-unknownbos-${indexUnknownDevCapability}`;
        const unknownBosItem = customTreeItem(`Unknown BOS Descriptor`, parentClassName);
        ...
        let offset = renderRawDataTree(unknownBosItem, rawDataByteElement, fields, rawData, originalOffset, this.rootElement_, parentClassName);
        const rawDataByteElements = rawDataByteElement.querySelectorAll('span');
        for (; offset < originalOffset + length; offset++) {
            rawDataByteElements[offset].classList.add(`field-offset-${offset}`);
            rawDataByteElements[offset].classList.add(parentClassName);
    ...
    async getUrlDescriptor_(rawData, offset, item) {
        ...
        try {
            ...
            const urlResponse = await this.usbDeviceProxy_.controlTransferIn(usbControlTransferParams, MAX_URL_DESCRIPTOR_LENGTH, CONTROL_TRANSFER_TIMEOUT_MS);
            checkTransferSuccess(urlResponse.status, 'Failed to read the device URL descriptor.', this.rootElement_);
            let url = '';
            ...
            landingPageItem.labelElement.addEventListener('click', () => window.open(url, '_blank'));
    ...
    async getMsOs20DescriptorSet_(vendorCode, msOs20DescriptorSetLength) {
        ...
        try {
            ...
            response = await this.usbDeviceProxy_.controlTransferIn(usbControlTransferParams, msOs20DescriptorSetLength, CONTROL_TRANSFER_TIMEOUT_MS);
            checkTransferSuccess(response.status, 'Failed to read the Microsoft OS 2.0 descriptor set.', this.rootElement_);
        }
        catch (e) {
            showError(e.message, this.rootElement_);
    ...
    async sendMsOs20DescriptorSetAltEnumCommand_(vendorCode, altEnumCode) {
        ...
        try {
            ...
            // of the request.
            const response = await this.usbDeviceProxy_.controlTransferOut(usbControlTransferParams, { buffer: [] }, CONTROL_TRANSFER_TIMEOUT_MS);
            checkTransferSuccess(response.status, 'Failed to read the Microsoft OS 2.0 descriptor ' +
                'alternate enumeration set.', this.rootElement_);
        }
        catch (e) {
            showError(e.message, this.rootElement_);
        }
    ...
    renderMsOs20DescriptorSet_(msOs20RawData) {
        const displayElement = addNewDescriptorDisplayElement(this.rootElement_, 'Microsoft OS 2.0 Descriptor Set');
        const rawDataTreeRoot = displayElement.rawDataTreeRoot;
        const rawDataByteElement = displayElement.rawDataByteElement;
        ...
        while (msOs20DescriptorOffset + MS_OS_20_DESCRIPTOR_TYPE_OFFSET + 1 <
            ...
            const msOs20DescriptorType = data.getUint16(msOs20DescriptorOffset + MS_OS_20_DESCRIPTOR_TYPE_OFFSET, true);
            switch (msOs20DescriptorType) {
                case MS_OS_20_SET_HEADER_DESCRIPTOR:
                    msOs20DescriptorOffset = this.renderMsOs20SetHeader_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset);
                    break;
                case MS_OS_20_SUBSET_HEADER_CONFIGURATION:
                    msOs20DescriptorOffset = this.renderMsOs20ConfigurationSubsetHeader_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset, indexMsOs20Descriptor);
                    indexMsOs20Descriptor++;
                    break;
                case MS_OS_20_SUBSET_HEADER_FUNCTION:
                    msOs20DescriptorOffset = this.renderMsOs20FunctionSubsetHeader_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset, indexMsOs20Descriptor);
                    indexMsOs20Descriptor++;
                    break;
                case MS_OS_20_FEATURE_COMPATIBLE_ID:
                    msOs20DescriptorOffset = this.renderMsOs20FeatureCompatibleId_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset, indexMsOs20Descriptor);
                    indexMsOs20Descriptor++;
                    break;
                case MS_OS_20_FEATURE_REG_PROPERTY:
                    msOs20DescriptorOffset = this.renderMsOs20FeatureRegistryProperty_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset, indexMsOs20Descriptor);
                    indexMsOs20Descriptor++;
                    break;
                case MS_OS_20_FEATURE_MIN_RESUME_TIME:
                    msOs20DescriptorOffset = this.renderMsOs20FeatureMinResumeTime_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset, indexMsOs20Descriptor);
                    indexMsOs20Descriptor++;
                    break;
                case MS_OS_20_FEATURE_MODEL_ID:
                    msOs20DescriptorOffset = this.renderMsOs20FeatureModelId_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset, indexMsOs20Descriptor);
                    indexMsOs20Descriptor++;
                    break;
                case MS_OS_20_FEATURE_CCGP_DEVICE:
                    msOs20DescriptorOffset = this.renderMsOs20FeatureCcgpDevice_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset, indexMsOs20Descriptor);
                    indexMsOs20Descriptor++;
                    break;
                case MS_OS_20_FEATURE_VENDOR_REVISION:
                    msOs20DescriptorOffset = this.renderMsOs20FeatureVendorRevision_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset, indexMsOs20Descriptor);
                    indexMsOs20Descriptor++;
                    break;
                default:
                    msOs20DescriptorOffset =
                        this.renderUnknownMsOs20DescriptorDescriptor_(rawDataTreeRoot, rawDataByteElement, msOs20RawData, msOs20DescriptorOffset, indexMsOs20Descriptor);
                    indexMsOs20Descriptor++;
            }
        }
        addMappingAction(rawDataTreeRoot, rawDataByteElement);
     ...
     */
    renderMsOs20SetHeader_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset) {
        const data = new DataView(rawData.buffer);
        const length = data.getUint16(originalOffset + MS_OS_20_DESCRIPTOR_LENGTH_OFFSET, true);
        const fields = [
            {
        ...
        const offset = renderRawDataTree(rawDataTreeRoot, rawDataByteElement, fields, rawData, originalOffset, this.rootElement_);
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 Descriptor ' +
                'Set header.', this.rootElement_);
        }
        return offset;
    }
    /**
     ...
     */
    renderMsOs20ConfigurationSubsetHeader_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexMsOs20Descriptor) {
        const parentClassName = `descriptor-ms-os-20-subdescriptor-${indexMsOs20Descriptor}`;
        const item = customTreeItem('Microsoft OS 2.0 Configuration Subset Header', parentClassName);
        ...
        const offset = renderRawDataTree(item, rawDataByteElement, fields, rawData, originalOffset, this.rootElement_, parentClassName);
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 ' +
                'Configuration Subset Header.', this.rootElement_);
        }
        return offset;
    }
    /**
     ...
     */
    renderMsOs20FunctionSubsetHeader_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexMsOs20Descriptor) {
        const parentClassName = `descriptor-ms-os-20-subdescriptor-${indexMsOs20Descriptor}`;
        const item = customTreeItem('Microsoft OS 2.0 Function Subset Header', parentClassName);
        rawDataTreeRoot.add(item);
        const data = new DataView(rawData.buffer);
        const length = data.getUint16(originalOffset + MS_OS_20_DESCRIPTOR_LENGTH_OFFSET, true);
        const fields = [
        ...
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 ' +
                'Function Subset Header.', this.rootElement_);
        }
        return offset;
    }
    /**
     ...
     */
    renderMsOs20FeatureCompatibleId_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexMsOs20Descriptor) {
        const parentClassName = `descriptor-ms-os-20-subdescriptor-${indexMsOs20Descriptor}`;
        const item = customTreeItem('Microsoft OS 2.0 Compatible ID Descriptor', parentClassName);
        rawDataTreeRoot.add(item);
        const data = new DataView(rawData.buffer);
        const length = data.getUint16(originalOffset + MS_OS_20_DESCRIPTOR_LENGTH_OFFSET, true);
        const fields = [
        ...
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 ' +
                'Compatible ID Descriptor.', this.rootElement_);
        }
        return offset;
    }
    /**
     ...
     */
    renderMsOs20FeatureRegistryProperty_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexMsOs20Descriptor) {
        const parentClassName = `descriptor-ms-os-20-subdescriptor-${indexMsOs20Descriptor}`;
        const item = customTreeItem('Microsoft OS 2.0 Registry Property Descriptor', parentClassName);
        rawDataTreeRoot.add(item);
        const data = new DataView(rawData.buffer);
        const length = data.getUint16(originalOffset + MS_OS_20_DESCRIPTOR_LENGTH_OFFSET, true);
        const featureRegistryPropertyDataType = data.getUint16(originalOffset +
            MS_OS_20_REGISTRY_PROPERTY_DESCRIPTOR_PROPERTY_DATA_TYPE_OFFSET, true);
        const propertyNameLength = data.getUint16(originalOffset +
            MS_OS_20_REGISTRY_PROPERTY_DESCRIPTOR_NAME_LENGTH_OFFSET, true);
        const fields = [
            {
                label: 'Length: ',
                size: 2,
            ...
            },
            {
                ...
                extraTreeItemFormatter: (rawData, offset, item, fieldLabel) => this.renderFeatureRegistryPropertyDataItem_(rawData, offset, item, fieldLabel, featureRegistryPropertyDataType, propertyNameLength),
            },
        ];
        let offset = renderRawDataTree(item, rawDataByteElement, fields, rawData, originalOffset, this.rootElement_, parentClassName);
        while (offset < originalOffset + length) {
            ...
            const propertyDataFields = [
                ...
                },
                {
                    ...
                    formatter: formatUnknown,
                    extraTreeItemFormatter: (rawData, offset, item, fieldLabel) => this.renderFeatureRegistryPropertyDataItem_(rawData, offset, item, fieldLabel, featureRegistryPropertyDataType, propertyDataLength),
                },
            ];
            offset = renderRawDataTree(item, rawDataByteElement, propertyDataFields, rawData, offset, this.rootElement_, parentClassName);
        }
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 ' +
                'Registry Property Descriptor.', this.rootElement_);
        }
        return offset;
    }
    /**
     ...
     */
    renderMsOs20FeatureMinResumeTime_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexMsOs20Descriptor) {
        const parentClassName = `descriptor-ms-os-20-subdescriptor-${indexMsOs20Descriptor}`;
        const item = customTreeItem('Microsoft OS 2.0 Minimum USB Resume Time Descriptor', parentClassName);
        rawDataTreeRoot.add(item);
        const data = new DataView(rawData.buffer);
        const length = data.getUint16(originalOffset + MS_OS_20_DESCRIPTOR_LENGTH_OFFSET, true);
        const fields = [
        ...
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 ' +
                'Minimum USB Resume Time Descriptor.', this.rootElement_);
        }
        return offset;
    }
    /**
     ...
     *     model ID descriptor.
     */
    renderMsOs20FeatureModelId_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexMsOs20Descriptor) {
        const parentClassName = `descriptor-ms-os-20-subdescriptor-${indexMsOs20Descriptor}`;
        const item = customTreeItem('Microsoft OS 2.0 Model ID Descriptor', parentClassName);
        rawDataTreeRoot.add(item);
        const data = new DataView(rawData.buffer);
        const length = data.getUint16(originalOffset + MS_OS_20_DESCRIPTOR_LENGTH_OFFSET, true);
        const fields = [
        ...
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 ' +
                'Model ID Descriptor.', this.rootElement_);
        }
        return offset;
    }
    /**
     ...
     */
    renderMsOs20FeatureCcgpDevice_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexMsOs20Descriptor) {
        const parentClassName = `descriptor-ms-os-20-subdescriptor-${indexMsOs20Descriptor}`;
        const item = customTreeItem('Microsoft OS 2.0 Common Class Generic Parent (CCGP) Device ' +
        ...
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 ' +
                'CCGP Device Descriptor.', this.rootElement_);
        }
        return offset;
    }
    /**
     ...
     */
    renderMsOs20FeatureVendorRevision_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexMsOs20Descriptor) {
        const parentClassName = `descriptor-ms-os-20-subdescriptor-${indexMsOs20Descriptor}`;
        const item = customTreeItem('Microsoft OS 2.0 Vendor Revision Descriptor', parentClassName);
        rawDataTreeRoot.add(item);
        const data = new DataView(rawData.buffer);
        const length = data.getUint16(originalOffset + MS_OS_20_DESCRIPTOR_LENGTH_OFFSET, true);
        const fields = [
        ...
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 ' +
                'Vendor Revision Descriptor.', this.rootElement_);
        }
        return offset;
    }
    /**
     ...
     *     OS 2.0 descriptor.
     */
    renderUnknownMsOs20DescriptorDescriptor_(rawDataTreeRoot, rawDataByteElement, rawData, originalOffset, indexDescriptor) {
        const parentClassName = `descriptor-ms-os-20-subdescriptor-${indexDescriptor}`;
        const item = customTreeItem('Microsoft OS 2.0 Descriptor Unknown Descriptor', parentClassName);
        rawDataTreeRoot.add(item);
        const data = new DataView(rawData.buffer);
        const length = data.getUint16(originalOffset + MS_OS_20_DESCRIPTOR_LENGTH_OFFSET, true);
        const fields = [
        ...
        const rawDataByteElements = rawDataByteElement.querySelectorAll('span');
        for (; offset < originalOffset + length; offset++) {
            rawDataByteElements[offset].classList.add(`field-offset-${offset}`, parentClassName);
        }
        if (offset !== originalOffset + length) {
            showError('An error occurred while rendering Microsoft OS 2.0 ' +
                'Unknown Descriptor.', this.rootElement_);
        }
        return offset;
    }
    /**
    ...
    async sendTestingRequest_(usbControlTransferParams, length, direction) {
        try {
            ...
            if (direction === 'Device-to-Host') {
                const response = await this.usbDeviceProxy_.controlTransferIn(usbControlTransferParams, length, CONTROL_TRANSFER_TIMEOUT_MS);
                checkTransferSuccess(response.status, 'Failed to send request.', this.rootElement_);
                ...
                const response = await this.usbDeviceProxy_.controlTransferOut(usbControlTransferParams, { buffer: data }, CONTROL_TRANSFER_TIMEOUT_MS);
                checkTransferSuccess(response.status, 'Failed to send request.', this.rootElement_);
            }
        }
        catch (e) {
    ...
    initialTestingToolPanel() {
        showWarn('Warning: This tool can send arbitrary commands to the device. ' +
            'Invalid commands may cause unexpected results.', this.rootElement_);
        ...
        const testingToolPanelInputTypeSelector = this.rootElement_.querySelector('#input-type');
        ...
        testingToolPanelInputTypeSelector.addEventListener('change', () => {
            ...
            const area = this.rootElement_.querySelector('#data-input-area');
            assert(area);
        ...
        inputTableRows.forEach((el, i) => {
            ...
            let directionInputElement = null;
            switch (i) {
            ...
            inputTableRow.querySelector('#query-length').addEventListener('blur', () => {
        ...
        buttons.forEach((button, i) => {
            button.addEventListener('click', () => {
                ...
                if (this.checkEnumParamValid_(type, 'Transfer Type', UsbControlTransferType) &&
                    this.checkEnumParamValid_(recipient, 'Transfer Recipient', UsbControlTransferRecipient) &&
                    ...
                    this.sendTestingRequest_(usbControlTransferParams, dataLength, direction);
                }
            });
        });
    }
...
function getRequestType(inputRow, inputType) {
    switch (inputType) {
        case INPUT_TYPE_DECIMAL_WITH_DROPDOWN:
            const select = inputRow.querySelector('#transfer-type');
            assert(select);
            return select.value;
        case INPUT_TYPE_HEX_BYTE:
            const input = inputRow.querySelector('#query-request-type');
            assert(input);
...
function getRequestTypeRecipient(inputRow, inputType) {
    switch (inputType) {
        case INPUT_TYPE_DECIMAL_WITH_DROPDOWN:
            const select = inputRow.querySelector('#transfer-recipient');
            assert(select);
            return select.value;
        case INPUT_TYPE_HEX_BYTE:
            const input = inputRow.querySelector('#query-request-type');
...
function getRequestTypeDirection(inputRow, inputType) {
    switch (inputType) {
        case INPUT_TYPE_DECIMAL_WITH_DROPDOWN:
            const select = inputRow.querySelector('#transfer-direction');
            assert(select);
            return select.value;
        case INPUT_TYPE_HEX_BYTE:
            const input = inputRow.querySelector('#query-request-type');
...
function getDecimalOrHex(inputRow, inputType, selector) {
    const input = inputRow.querySelector(selector);
    switch (inputType) {
        case INPUT_TYPE_DECIMAL_WITH_DROPDOWN:
            assert(input);
 ...
 */
function addNewDescriptorDisplayElement(rootElement, descriptorPanelTitle) {
    const descriptorPanelTemplate = rootElement.getRootNode()
        .querySelector('#descriptor-panel-template');
    assert(descriptorPanelTemplate);
    const descriptorPanelClone = document.importNode(descriptorPanelTemplate.content, true);
    const rawDataTreeRoot = descriptorPanelClone.querySelector('.raw-data-tree-view');
    assert(rawDataTreeRoot);
    const rawDataByteElement = descriptorPanelClone.querySelector('.raw-data-byte-view');
    assert(rawDataByteElement);
    rawDataTreeRoot.detail = { payload: {}, children: {} };
    if (descriptorPanelTitle) {
        const descriptorPanelTitleTemplate = rootElement.getRootNode()
            .querySelector('#descriptor-panel-title');
        assert(descriptorPanelTitleTemplate);
        const clone = document.importNode(descriptorPanelTitleTemplate.content, true)
 ...
 */
function customTreeItem(itemLabel, className) {
...
function addMappingAction(rawDataTreeRoot, rawDataByteElement) {
    ...
    function mapElement(el) {
        ...
        assert(fieldOffsetOrDescriptorClass.startsWith('field-offset-') ||
    ...
    rawDataByteElements.forEach((el) => {
        ...
        el.addEventListener('pointerenter', () => {
            rawDataByteElement.querySelectorAll(`.${fieldOffsetClass}`)
                .forEach((el) => el.classList.add('hovered-field'));
            configureMatchingItem(fieldOffsetClass, el => el.forceHoverStyle(true), rawDataTreeRoot);
        });
        el.addEventListener('pointerleave', () => {
            rawDataByteElement.querySelectorAll(`.${fieldOffsetClass}`)
                .forEach((el) => el.classList.remove('hovered-field'));
            configureMatchingItem(fieldOffsetClass, el => el.forceHoverStyle(false), rawDataTreeRoot);
        });
        el.addEventListener('click', () => {
            configureMatchingItem(fieldOffsetClass, el => el.rowElement.click(), rawDataTreeRoot);
        });
    });
}
/**
 ...
 *     rendered.
 */
function renderRawDataTree(root, rawDataByteElement, fields, rawData, offset, rootElement, ...parentClassNames) {
    const rawDataByteElements = rawDataByteElement.querySelectorAll('span');
    for (const field of fields) {
        const className = `field-offset-${offset}`;
        let item;
        try {
            item = customTreeItem(`${field.label}${field.formatter(rawData, offset)}`, className);
        ...
        try {
            ...
            showError(`Error at rendering field at index ${offset}: ${message}`, rootElement);
        }
        offset += field.size;
    }
    return offset;
...
function renderRawDataBytes(rawDataByteElement, rawData) {
    const rawDataByteContainerTemplate = rawDataByteElement.getRootNode()
        .querySelector('#raw-data-byte-container-template');
    assert(rawDataByteContainerTemplate);
    const rawDataByteContainerClone = document.importNode(rawDataByteContainerTemplate.content, true);
    const rawDataByteContainerElement = rawDataByteContainerClone.querySelector('div');
    assert(rawDataByteContainerElement);
    const rawDataByteTemplate = rawDataByteElement.getRootNode()
        .querySelector('#raw-data-byte-template');
    assert(rawDataByteTemplate);
    for (const value of rawData) {
        const rawDataByteClone = document.importNode(rawDataByteTemplate.content, true);
...
function formatUsbVersion(rawData, offset) {
    return `${rawData[offset + 1]}.${rawData[offset] >> 4}.${rawData[offset] & 0x0F}`;
}
/**
 ...
 */
function formatUnknown(_rawData, _offset) {
    return '';
}
/**
