...
function createRow(rowInfo) {
    const template = document.querySelector('#datarow');
    assert(template);
    ...
    td[6].querySelectorAll('.engagement-bar')[0].style.width =
...
function createConfigRow(name, value) {
    const template = document.querySelector('#configrow');
    assert(template);
    const td = template.content.querySelectorAll('td');
...
function renderConfigTable(config) {
    ...
    configTableBody.innerHTML = window.trustedTypes ?
        window.trustedTypes.emptyHTML :
        '';
    configTableBody.appendChild(createConfigRow('Min Sessions', config.scoreMinVisits));
    configTableBody.appendChild(createConfigRow('Lower Threshold', config.highScoreLowerThreshold));
    configTableBody.appendChild(createConfigRow('Upper Threshold', config.highScoreUpperThreshold));
    configTableBody.appendChild(createConfigRow('Record MEI data', formatFeatureFlag(config.featureRecordData)));
    configTableBody.appendChild(createConfigRow('Bypass autoplay based on MEI', formatFeatureFlag(config.featureBypassAutoplay)));
    configTableBody.appendChild(createConfigRow('Preload MEI data', formatFeatureFlag(config.featurePreloadData)));
    configTableBody.appendChild(createConfigRow('MEI for HTTPS only', formatFeatureFlag(config.featureHttpsOnly)));
    configTableBody.appendChild(createConfigRow('Autoplay disable settings', formatFeatureFlag(config.featureAutoplayDisableSettings)));
    configTableBody.appendChild(createConfigRow('Unified autoplay (preference)', formatFeatureFlag(config.prefDisableUnifiedAutoplay)));
    configTableBody.appendChild(createConfigRow('Custom autoplay policy', formatFeatureFlag(config.hasCustomAutoplayPolicy)));
    configTableBody.appendChild(createConfigRow('Autoplay Policy', config.autoplayPolicy));
    configTableBody.appendChild(createConfigRow('Preload version', config.preloadVersion ? config.preloadVersion : 'Not Available'));
}
/**
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    engagementTableBody =
        document.querySelector('#engagement-table-body');
    configTableBody = document.querySelector('#config-table-body');
    // Set table header sort handlers.
    const engagementTableHeader = document.querySelector('#engagement-table-header');
    ...
    const copyAllToClipboardButton = document.querySelector('#copy-all-to-clipboard');
    ...
    const showNoPlaybacksCheckbox = document.querySelector('#show-no-playbacks');
