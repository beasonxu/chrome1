...
function updateWebRtcTextLogsList(textLogsList, version) {
    const banner = document.querySelector('#text-log-banner');
    assert(banner);
    banner.textContent =
        loadTimeData.getStringF('webrtcTextLogCountFormat', textLogsList.length);
    const textLogSection = document.querySelector('#text-log-list');
    ...
    for (let i = 0; i < textLogsList.length; i++) {
        ...
        title.textContent = loadTimeData.getStringF('webrtcLogHeaderFormat', textLog['capture_time']);
        logBlock.appendChild(title);
        const localFileLine = document.createElement('p');
        const localFile = textLog['local_file'];
        ...
        if (id.length === 0) {
            ...
            uploadLine.textContent =
                loadTimeData.getStringF('webrtcLogUploadTimeFormat', textLog['upload_time']) +
                    '. ' +
    ...
    const noLogs = document.querySelector('#text-no-logs');
    assert(noLogs);
    noLogs.hidden = (textLogsList.length !== 0);
}
function updateWebRtcEventLogsList(eventLogsList) {
    const eventLogSection = document.querySelector('#event-log-list');
    ...
    const banner = document.querySelector('#event-log-banner');
    assert(banner);
    banner.textContent =
        loadTimeData.getStringF('webrtcEventLogCountFormat', entries);
    const noLogs = document.querySelector('#event-no-logs');
    assert(noLogs);
...
function createUploadUnsuccessfulEventLogEntryElement(eventLogEntry) {
    ...
    appendLocalLogId(logBlock, eventLogEntry);
    const uploadLine = document.createElement('p');
    uploadLine.textContent = loadTimeData.getStringF('webrtcLogFailedUploadTimeFormat', eventLogEntry['upload_time']);
    logBlock.appendChild(uploadLine);
    return logBlock;
}
function createUploadSuccessfulEventLogEntryElement(eventLogEntry) {
    const expectedFields = ['capture_time', 'local_id', 'upload_id', 'upload_time'];
    if (!verifyExpectedFields(eventLogEntry, expectedFields)) {
        return null;
    }
    const logBlock = document.createElement('div');
    ...
    const uploadLine = document.createElement('p');
    uploadLine.textContent =
        loadTimeData.getStringF('webrtcLogUploadTimeFormat', eventLogEntry['upload_time']) +
            '. ' +
            loadTimeData.getStringF('webrtcLogReportIdFormat', eventLogEntry['upload_id']) +
            '. ';
    logBlock.appendChild(uploadLine);
    return logBlock;
}
...
function appendCaptureTime(logBlock, eventLogEntry) {
    const title = document.createElement('h3');
    title.textContent = loadTimeData.getStringF('webrtcLogHeaderFormat', eventLogEntry['capture_time']);
    logBlock.appendChild(title);
}
function appendLocalFile(logBlock, eventLogEntry) {
    ...
    localFileLine.textContent =
        loadTimeData.getString('webrtcLogLocalFileLabelFormat') + ' ';
    const localFileLink = document.createElement('a');
    const fileName = eventLogEntry['local_file'];
...
function appendLocalLogId(logBlock, eventLogEntry) {
    ...
    localIdLine.textContent =
        loadTimeData.getStringF('webrtcEventLogLocalLogIdFormat', eventLogEntry['local_id']) +
            '';
