...
void CompositorFrameSinkProxy::SetThreadIds(
    const WTF::Vector<int32_t>& in_thread_ids) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1(
...
std::move(p_type));
    ...
    case internal::kCompositorFrameSink_SetThreadIds_Name: {
      ...
      bool success = true;
      WTF::Vector<int32_t> p_thread_ids{};
...
void CompositorFrameSinkInterceptorForTesting::SetThreadIds(const WTF::Vector<int32_t>& thread_ids) {
...
,
             WTF::Vector<::viz::ReturnedResource> resources) {*out_resources = std::move(resources);
            loop->Quit();
          },
          &loop,
          out_resources));
