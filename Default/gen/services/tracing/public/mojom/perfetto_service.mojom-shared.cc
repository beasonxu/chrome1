...
static NOINLINE const char* BufferFillPolicyToStringHelper(BufferFillPolicy value) {
...
static NOINLINE const char* TracingClientPriorityToStringHelper(TracingClientPriority value) {
  // Defined in a helper function to ensure that Clang generates a lookup table.
  switch(value) {
...
bool DataSourceConfig_Data::Validate(
  ...
  const mojo::internal::ContainerValidateParams track_event_config_raw_validate_params(
      0, false, nullptr);
  if (!mojo::internal::ValidateContainer(object->track_event_config_raw, validation_context,
                                         &track_event_config_raw_validate_params)) {
    return false;
  }
