// Copyright 2014 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.


// This file is autogenerated by
//     base/android/jni_generator/jni_generator.py
// For
//     org/chromium/content/browser/selection/SelectionPopupControllerImpl

#ifndef org_chromium_content_browser_selection_SelectionPopupControllerImpl_JNI
#define org_chromium_content_browser_selection_SelectionPopupControllerImpl_JNI

#include <jni.h>

#include "../../../../../../../base/android/jni_generator/jni_generator_helper.h"


// Step 1: Forward declarations.

JNI_REGISTRATION_EXPORT extern const char
    kClassPath_org_chromium_content_browser_selection_SelectionPopupControllerImpl[];
const char kClassPath_org_chromium_content_browser_selection_SelectionPopupControllerImpl[] =
    "org/chromium/content/browser/selection/SelectionPopupControllerImpl";
// Leaking this jclass as we cannot use LazyInstance from some threads.
JNI_REGISTRATION_EXPORT std::atomic<jclass>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(nullptr);
#ifndef org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz_defined
#define org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz_defined
inline jclass org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(JNIEnv* env)
    {
  return base::android::LazyGetClass(env,
      kClassPath_org_chromium_content_browser_selection_SelectionPopupControllerImpl,
      &g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz);
}
#endif


// Step 2: Constants (optional).


// Step 3: Method stubs.
namespace content {

static jlong JNI_SelectionPopupControllerImpl_Init(JNIEnv* env, const
    base::android::JavaParamRef<jobject>& caller,
    const base::android::JavaParamRef<jobject>& webContents);

JNI_GENERATOR_EXPORT jlong Java_J_N_MJHXNa8U(
    JNIEnv* env,
    jclass jcaller,
    jobject caller,
    jobject webContents) {
  return JNI_SelectionPopupControllerImpl_Init(env, base::android::JavaParamRef<jobject>(env,
      caller), base::android::JavaParamRef<jobject>(env, webContents));
}

JNI_GENERATOR_EXPORT void Java_J_N_M01adZlM(
    JNIEnv* env,
    jclass jcaller,
    jlong nativeSelectionPopupController,
    jobject caller,
    jboolean hidden) {
  SelectionPopupController* native =
      reinterpret_cast<SelectionPopupController*>(nativeSelectionPopupController);
  CHECK_NATIVE_PTR(env, jcaller, native, "SetTextHandlesTemporarilyHidden");
  return native->SetTextHandlesTemporarilyHidden(env, base::android::JavaParamRef<jobject>(env,
      caller), hidden);
}


static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_showSelectionMenu(nullptr);
static void Java_SelectionPopupControllerImpl_showSelectionMenu(JNIEnv* env, const
    base::android::JavaRef<jobject>& obj, JniIntWrapper left,
    JniIntWrapper top,
    JniIntWrapper right,
    JniIntWrapper bottom,
    JniIntWrapper handleHeight,
    jboolean isEditable,
    jboolean isPasswordType,
    const base::android::JavaRef<jstring>& selectionText,
    JniIntWrapper selectionStartOffset,
    jboolean canSelectAll,
    jboolean canRichlyEdit,
    jboolean shouldSuggest,
    JniIntWrapper sourceType,
    const base::android::JavaRef<jobject>& renderFrameHost) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env));

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "showSelectionMenu",
          "(IIIIIZZLjava/lang/String;IZZZILorg/chromium/content_public/browser/RenderFrameHost;)V",
          &g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_showSelectionMenu);

     env->CallVoidMethod(obj.obj(),
          call_context.base.method_id, as_jint(left), as_jint(top), as_jint(right), as_jint(bottom),
              as_jint(handleHeight), isEditable, isPasswordType, selectionText.obj(),
              as_jint(selectionStartOffset), canSelectAll, canRichlyEdit, shouldSuggest,
              as_jint(sourceType), renderFrameHost.obj());
}

static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_restoreSelectionPopupsIfNecessary(nullptr);
static void Java_SelectionPopupControllerImpl_restoreSelectionPopupsIfNecessary(JNIEnv* env, const
    base::android::JavaRef<jobject>& obj) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env));

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "restoreSelectionPopupsIfNecessary",
          "()V",
&g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_restoreSelectionPopupsIfNecessary);

     env->CallVoidMethod(obj.obj(),
          call_context.base.method_id);
}

static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onSelectionEvent(nullptr);
static void Java_SelectionPopupControllerImpl_onSelectionEvent(JNIEnv* env, const
    base::android::JavaRef<jobject>& obj, JniIntWrapper eventType,
    JniIntWrapper left,
    JniIntWrapper top,
    JniIntWrapper right,
    JniIntWrapper bottom) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env));

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "onSelectionEvent",
          "(IIIII)V",
          &g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onSelectionEvent);

     env->CallVoidMethod(obj.obj(),
          call_context.base.method_id, as_jint(eventType), as_jint(left), as_jint(top),
              as_jint(right), as_jint(bottom));
}

static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onDragUpdate(nullptr);
static void Java_SelectionPopupControllerImpl_onDragUpdate(JNIEnv* env, const
    base::android::JavaRef<jobject>& obj, JniIntWrapper type,
    jfloat x,
    jfloat y) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env));

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "onDragUpdate",
          "(IFF)V",
          &g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onDragUpdate);

     env->CallVoidMethod(obj.obj(),
          call_context.base.method_id, as_jint(type), x, y);
}

static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_getContext(nullptr);
static base::android::ScopedJavaLocalRef<jobject>
    Java_SelectionPopupControllerImpl_getContext(JNIEnv* env, const base::android::JavaRef<jobject>&
    obj) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env), NULL);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "getContext",
          "()Landroid/content/Context;",
          &g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_getContext);

  jobject ret =
      env->CallObjectMethod(obj.obj(),
          call_context.base.method_id);
  return base::android::ScopedJavaLocalRef<jobject>(env, ret);
}

static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onSelectionChanged(nullptr);
static void Java_SelectionPopupControllerImpl_onSelectionChanged(JNIEnv* env, const
    base::android::JavaRef<jobject>& obj, const base::android::JavaRef<jstring>& text) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env));

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "onSelectionChanged",
          "(Ljava/lang/String;)V",
&g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onSelectionChanged);

     env->CallVoidMethod(obj.obj(),
          call_context.base.method_id, text.obj());
}

static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onSelectAroundCaretSuccess(nullptr);
static void Java_SelectionPopupControllerImpl_onSelectAroundCaretSuccess(JNIEnv* env, const
    base::android::JavaRef<jobject>& obj, JniIntWrapper extendedStartAdjust,
    JniIntWrapper extendedEndAdjust,
    JniIntWrapper wordStartAdjust,
    JniIntWrapper wordEndAdjust) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env));

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "onSelectAroundCaretSuccess",
          "(IIII)V",
&g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onSelectAroundCaretSuccess);

     env->CallVoidMethod(obj.obj(),
          call_context.base.method_id, as_jint(extendedStartAdjust), as_jint(extendedEndAdjust),
              as_jint(wordStartAdjust), as_jint(wordEndAdjust));
}

static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onSelectAroundCaretFailure(nullptr);
static void Java_SelectionPopupControllerImpl_onSelectAroundCaretFailure(JNIEnv* env, const
    base::android::JavaRef<jobject>& obj) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env));

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "onSelectAroundCaretFailure",
          "()V",
&g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_onSelectAroundCaretFailure);

     env->CallVoidMethod(obj.obj(),
          call_context.base.method_id);
}

static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_hidePopupsAndPreserveSelection(nullptr);
static void Java_SelectionPopupControllerImpl_hidePopupsAndPreserveSelection(JNIEnv* env, const
    base::android::JavaRef<jobject>& obj) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env));

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "hidePopupsAndPreserveSelection",
          "()V",
&g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_hidePopupsAndPreserveSelection);

     env->CallVoidMethod(obj.obj(),
          call_context.base.method_id);
}

static std::atomic<jmethodID>
    g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_nativeSelectionPopupControllerDestroyed(nullptr);
static void Java_SelectionPopupControllerImpl_nativeSelectionPopupControllerDestroyed(JNIEnv* env,
    const base::android::JavaRef<jobject>& obj) {
  jclass clazz = org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env);
  CHECK_CLAZZ(env, obj.obj(),
      org_chromium_content_browser_selection_SelectionPopupControllerImpl_clazz(env));

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_INSTANCE>(
          env,
          clazz,
          "nativeSelectionPopupControllerDestroyed",
          "()V",
&g_org_chromium_content_browser_selection_SelectionPopupControllerImpl_nativeSelectionPopupControllerDestroyed);

     env->CallVoidMethod(obj.obj(),
          call_context.base.method_id);
}

}  // namespace content

#endif  // org_chromium_content_browser_selection_SelectionPopupControllerImpl_JNI
