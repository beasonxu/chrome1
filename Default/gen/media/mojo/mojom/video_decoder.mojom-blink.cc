    ...
    [&](perfetto::TracedValue context){
      ...
      perfetto::WriteIntoTracedValueWithFallback(
           dict.AddItem("supported_configs"), in_supported_configs,
                        "<value of type WTF::Vector<SupportedVideoDecoderConfigPtr>>");
      perfetto::WriteIntoTracedValueWithFallback(
...
void VideoDecoderInterceptorForTesting::Initialize(::media::mojom::blink::VideoDecoderConfigPtr config, bool low_delay, const absl::optional<::base::UnguessableToken>& cdm_id, InitializeCallback callback) {
...
void VideoDecoderAsyncWaiter::GetSupportedConfigs(
  ...
  proxy_->GetSupportedConfigs(
      base::BindOnce(
          [](base::RunLoop* loop,
             WTF::Vector<SupportedVideoDecoderConfigPtr>* out_supported_configs
,
             ::media::mojom::blink::VideoDecoderType* out_decoder_type
,
             WTF::Vector<SupportedVideoDecoderConfigPtr> supported_configs,
             ::media::mojom::blink::VideoDecoderType decoder_type) {*out_supported_configs = std::move(supported_configs);*out_decoder_type = std::move(decoder_type);
            loop->Quit();
          },
          &loop,
...
,
             ...
             ::media::mojom::blink::VideoDecoderType decoder_type) {*out_status = std::move(status);*out_needs_bitstream_conversion = std::move(needs_bitstream_conversion);*out_max_decode_requests = std::move(max_decode_requests);*out_decoder_type = std::move(decoder_type);
            loop->Quit();
          },
          &loop,
          out_status,
...
void VideoDecoderClientInterceptorForTesting::OnVideoFrameDecoded(::media::mojom::blink::VideoFramePtr frame, bool can_read_without_stalling, const absl::optional<::base::UnguessableToken>& release_token) {
