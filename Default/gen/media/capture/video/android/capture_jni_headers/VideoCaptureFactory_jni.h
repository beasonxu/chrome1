// Copyright 2014 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.


// This file is autogenerated by
//     base/android/jni_generator/jni_generator.py
// For
//     org/chromium/media/VideoCaptureFactory

#ifndef org_chromium_media_VideoCaptureFactory_JNI
#define org_chromium_media_VideoCaptureFactory_JNI

#include <jni.h>

#include "../../../../../../../../base/android/jni_generator/jni_generator_helper.h"


// Step 1: Forward declarations.

JNI_REGISTRATION_EXPORT extern const char kClassPath_org_chromium_media_VideoCaptureFactory[];
const char kClassPath_org_chromium_media_VideoCaptureFactory[] =
    "org/chromium/media/VideoCaptureFactory";
// Leaking this jclass as we cannot use LazyInstance from some threads.
JNI_REGISTRATION_EXPORT std::atomic<jclass> g_org_chromium_media_VideoCaptureFactory_clazz(nullptr);
#ifndef org_chromium_media_VideoCaptureFactory_clazz_defined
#define org_chromium_media_VideoCaptureFactory_clazz_defined
inline jclass org_chromium_media_VideoCaptureFactory_clazz(JNIEnv* env) {
  return base::android::LazyGetClass(env, kClassPath_org_chromium_media_VideoCaptureFactory,
      &g_org_chromium_media_VideoCaptureFactory_clazz);
}
#endif


// Step 2: Constants (optional).


// Step 3: Method stubs.
namespace media {


static std::atomic<jmethodID>
    g_org_chromium_media_VideoCaptureFactory_isLegacyOrDeprecatedDevice(nullptr);
static jboolean Java_VideoCaptureFactory_isLegacyOrDeprecatedDevice(JNIEnv* env, JniIntWrapper id) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), false);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "isLegacyOrDeprecatedDevice",
          "(I)Z",
          &g_org_chromium_media_VideoCaptureFactory_isLegacyOrDeprecatedDevice);

  jboolean ret =
      env->CallStaticBooleanMethod(clazz,
          call_context.base.method_id, as_jint(id));
  return ret;
}

static std::atomic<jmethodID> g_org_chromium_media_VideoCaptureFactory_createVideoCapture(nullptr);
static base::android::ScopedJavaLocalRef<jobject>
    Java_VideoCaptureFactory_createVideoCapture(JNIEnv* env, JniIntWrapper id,
    jlong nativeVideoCaptureDeviceAndroid) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), NULL);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "createVideoCapture",
          "(IJ)Lorg/chromium/media/VideoCapture;",
          &g_org_chromium_media_VideoCaptureFactory_createVideoCapture);

  jobject ret =
      env->CallStaticObjectMethod(clazz,
          call_context.base.method_id, as_jint(id), nativeVideoCaptureDeviceAndroid);
  return base::android::ScopedJavaLocalRef<jobject>(env, ret);
}

static std::atomic<jmethodID> g_org_chromium_media_VideoCaptureFactory_getNumberOfCameras(nullptr);
static jint Java_VideoCaptureFactory_getNumberOfCameras(JNIEnv* env) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), 0);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getNumberOfCameras",
          "()I",
          &g_org_chromium_media_VideoCaptureFactory_getNumberOfCameras);

  jint ret =
      env->CallStaticIntMethod(clazz,
          call_context.base.method_id);
  return ret;
}

static std::atomic<jmethodID> g_org_chromium_media_VideoCaptureFactory_getCaptureApiType(nullptr);
static jint Java_VideoCaptureFactory_getCaptureApiType(JNIEnv* env, JniIntWrapper index) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), 0);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getCaptureApiType",
          "(I)I",
          &g_org_chromium_media_VideoCaptureFactory_getCaptureApiType);

  jint ret =
      env->CallStaticIntMethod(clazz,
          call_context.base.method_id, as_jint(index));
  return ret;
}

static std::atomic<jmethodID> g_org_chromium_media_VideoCaptureFactory_isZoomSupported(nullptr);
static jboolean Java_VideoCaptureFactory_isZoomSupported(JNIEnv* env, JniIntWrapper index) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), false);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "isZoomSupported",
          "(I)Z",
          &g_org_chromium_media_VideoCaptureFactory_isZoomSupported);

  jboolean ret =
      env->CallStaticBooleanMethod(clazz,
          call_context.base.method_id, as_jint(index));
  return ret;
}

static std::atomic<jmethodID> g_org_chromium_media_VideoCaptureFactory_getFacingMode(nullptr);
static jint Java_VideoCaptureFactory_getFacingMode(JNIEnv* env, JniIntWrapper index) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), 0);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getFacingMode",
          "(I)I",
          &g_org_chromium_media_VideoCaptureFactory_getFacingMode);

  jint ret =
      env->CallStaticIntMethod(clazz,
          call_context.base.method_id, as_jint(index));
  return ret;
}

static std::atomic<jmethodID> g_org_chromium_media_VideoCaptureFactory_getDeviceId(nullptr);
static base::android::ScopedJavaLocalRef<jstring> Java_VideoCaptureFactory_getDeviceId(JNIEnv* env,
    JniIntWrapper index) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), NULL);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getDeviceId",
          "(I)Ljava/lang/String;",
          &g_org_chromium_media_VideoCaptureFactory_getDeviceId);

  jstring ret =
      static_cast<jstring>(env->CallStaticObjectMethod(clazz,
          call_context.base.method_id, as_jint(index)));
  return base::android::ScopedJavaLocalRef<jstring>(env, ret);
}

static std::atomic<jmethodID> g_org_chromium_media_VideoCaptureFactory_getDeviceName(nullptr);
static base::android::ScopedJavaLocalRef<jstring> Java_VideoCaptureFactory_getDeviceName(JNIEnv*
    env, JniIntWrapper index) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), NULL);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getDeviceName",
          "(I)Ljava/lang/String;",
          &g_org_chromium_media_VideoCaptureFactory_getDeviceName);

  jstring ret =
      static_cast<jstring>(env->CallStaticObjectMethod(clazz,
          call_context.base.method_id, as_jint(index)));
  return base::android::ScopedJavaLocalRef<jstring>(env, ret);
}

static std::atomic<jmethodID>
    g_org_chromium_media_VideoCaptureFactory_getDeviceSupportedFormats(nullptr);
static base::android::ScopedJavaLocalRef<jobjectArray>
    Java_VideoCaptureFactory_getDeviceSupportedFormats(JNIEnv* env, JniIntWrapper index) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), NULL);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getDeviceSupportedFormats",
          "(I)[Lorg/chromium/media/VideoCaptureFormat;",
          &g_org_chromium_media_VideoCaptureFactory_getDeviceSupportedFormats);

  jobjectArray ret =
      static_cast<jobjectArray>(env->CallStaticObjectMethod(clazz,
          call_context.base.method_id, as_jint(index)));
  return base::android::ScopedJavaLocalRef<jobjectArray>(env, ret);
}

static std::atomic<jmethodID>
    g_org_chromium_media_VideoCaptureFactory_getCaptureFormatWidth(nullptr);
static jint Java_VideoCaptureFactory_getCaptureFormatWidth(JNIEnv* env, const
    base::android::JavaRef<jobject>& format) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), 0);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getCaptureFormatWidth",
          "(Lorg/chromium/media/VideoCaptureFormat;)I",
          &g_org_chromium_media_VideoCaptureFactory_getCaptureFormatWidth);

  jint ret =
      env->CallStaticIntMethod(clazz,
          call_context.base.method_id, format.obj());
  return ret;
}

static std::atomic<jmethodID>
    g_org_chromium_media_VideoCaptureFactory_getCaptureFormatHeight(nullptr);
static jint Java_VideoCaptureFactory_getCaptureFormatHeight(JNIEnv* env, const
    base::android::JavaRef<jobject>& format) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), 0);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getCaptureFormatHeight",
          "(Lorg/chromium/media/VideoCaptureFormat;)I",
          &g_org_chromium_media_VideoCaptureFactory_getCaptureFormatHeight);

  jint ret =
      env->CallStaticIntMethod(clazz,
          call_context.base.method_id, format.obj());
  return ret;
}

static std::atomic<jmethodID>
    g_org_chromium_media_VideoCaptureFactory_getCaptureFormatFramerate(nullptr);
static jint Java_VideoCaptureFactory_getCaptureFormatFramerate(JNIEnv* env, const
    base::android::JavaRef<jobject>& format) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), 0);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getCaptureFormatFramerate",
          "(Lorg/chromium/media/VideoCaptureFormat;)I",
          &g_org_chromium_media_VideoCaptureFactory_getCaptureFormatFramerate);

  jint ret =
      env->CallStaticIntMethod(clazz,
          call_context.base.method_id, format.obj());
  return ret;
}

static std::atomic<jmethodID>
    g_org_chromium_media_VideoCaptureFactory_getCaptureFormatPixelFormat(nullptr);
static jint Java_VideoCaptureFactory_getCaptureFormatPixelFormat(JNIEnv* env, const
    base::android::JavaRef<jobject>& format) {
  jclass clazz = org_chromium_media_VideoCaptureFactory_clazz(env);
  CHECK_CLAZZ(env, clazz,
      org_chromium_media_VideoCaptureFactory_clazz(env), 0);

  jni_generator::JniJavaCallContextChecked call_context;
  call_context.Init<
      base::android::MethodID::TYPE_STATIC>(
          env,
          clazz,
          "getCaptureFormatPixelFormat",
          "(Lorg/chromium/media/VideoCaptureFormat;)I",
          &g_org_chromium_media_VideoCaptureFactory_getCaptureFormatPixelFormat);

  jint ret =
      env->CallStaticIntMethod(clazz,
          call_context.base.method_id, format.obj());
  return ret;
}

}  // namespace media

#endif  // org_chromium_media_VideoCaptureFactory_JNI
